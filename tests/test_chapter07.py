...
from pyspark.sql.functions import col, when, lit, broadcast, spark_partition_id, monotonically_increasing_id
...
class TestPartitioning:
    ...
    def test_default_partitioning(self, spark_session):
        ...
        # 檢查數據分佈
        partition_sizes = df.rdd.mapPartitions(lambda iterator: [len(list(iterator))]).collect()
    ...
    def test_hash_partitioning(self, spark_session):
        ...
        # 添加分區ID列
        with_partition_id = hash_partitioned_df.withColumn("partition_id", spark_partition_id())
        
        # 檢查每個category的分區分佈
        partition_distribution = with_partition_id.groupBy("category", "partition_id").count().collect()
        ...
        for category, partitions in category_partitions.items():
            assert len(partitions) == 1, f"Category {category} spans multiple partitions: {partitions}"
        ...
        for year in [2022, 2023, 2024]:
            for month in range(1, 13):
                for day in range(1, 11):  # 每月10天
                    data.append((f"{year}-{month:02d}-{day:02d}", year, month, day, day * 100))
        
        df = spark_session.createDataFrame(data, ["date", "year", "month", "day", "sales"])
...
class TestCaching:
    ...
    def test_checkpoint(self, spark_session, temp_dir):
        ...
        # 進行複雜的轉換
        transformed_df = df.filter(col("category") < 5) \
...
class TestJoinOptimization:
    ...
    def test_broadcast_join(self, spark_session):
        ...
        large_df = spark_session.createDataFrame(large_data, ["user_id", "username", "department_id"])
        ...
        # 使用廣播連接
        broadcast_joined = large_df.join(broadcast(small_df), 
                                       large_df.department_id == small_df.dept_id)
    ...
    def test_bucket_join(self, spark_session, temp_dir):
        ...
        orders_df = spark_session.createDataFrame(orders_data, ["order_id", "customer_id", "order_desc", "amount"])
        
        customers_data = [(i, f"customer_{i}", f"city_{i % 20}") for i in range(100)]
        customers_df = spark_session.createDataFrame(customers_data, ["customer_id", "customer_name", "city"])
    ...
    def test_join_reordering(self, spark_session):
        ...
        # 多表連接
        result = table_a.join(table_b, "id") \
                       .join(table_c, "group_id")
...
class TestResourceManagement:
    ...
    def test_executor_memory_usage(self, spark_session):
        """測試執行器內存使用"""
        # 創建內存密集型操作
        large_data = [(i, f"data_{i}" * 100, [j for j in range(100)]) for i in range(1000)]
    ...
    def test_task_parallelism(self, spark_session):
        ...
        # 執行並行操作
        result = partitioned_df.filter(col("a") > 5000) \
...
class TestQueryOptimization:
    ...
    def test_column_pruning(self, spark_session):
        """測試列裁剪"""
        # 創建多列數據
        data = [(i, f"name_{i}", f"email_{i}@test.com", i * 100, i % 10, 
                 f"address_{i}", f"phone_{i}", f"comment_{i}") 
                for i in range(1000)]
        
        columns = ["id", "name", "email", "salary", "department", "address", "phone", "comments"]
    ...
    def test_constant_folding(self, spark_session):
        ...
        # 使用常量表達式
        result_df = df.withColumn("constant_calc", lit(10) + lit(20)) \
                     .withColumn("mixed_calc", col("a") + lit(100)) \
                     .filter(lit(True))  # 常量過濾條件
    ...
    def test_join_reordering_cost_based(self, spark_session):
        ...
        medium_table = spark_session.createDataFrame(
            [(i, f"medium_{i}", i % 10) for i in range(100)], ["id", "data_medium", "small_id"]
        )
        
        large_table = spark_session.createDataFrame(
            [(i, f"large_{i}", i % 100) for i in range(1000)], ["id", "data_large", "medium_id"]
        )
        
        # 多表連接 - Spark應該優化連接順序
        result = large_table.join(medium_table, large_table.medium_id == medium_table.id) \
...
class TestPerformanceMonitoring:
    ...
    def test_memory_usage_monitoring(self, spark_session):
        """測試內存使用監控"""
        # 創建內存密集型操作
        large_data = [(i, f"text_{i}" * 1000, [j for j in range(100)]) for i in range(1000)]
    ...
    def test_explain_plan(self, spark_session):
        ...
        # 創建查詢
        query = df.filter(col("value") > 50000) \
                 .groupBy("category") \
                 .count() \
                 .orderBy("count")
