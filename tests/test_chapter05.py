...
from pyspark.sql.functions import (
    col, when, lit, avg, sum as spark_sum, count, max as spark_max,
    window, current_timestamp, to_timestamp, explode, split
)
...
class TestStreamingBasics:
    ...
    def test_create_stream_from_rate_source(self, streaming_spark_session):
        ...
        rate_stream = streaming_spark_session \
            .readStream \
            .format("rate") \
    ...
    def test_create_stream_from_socket(self, streaming_spark_session):
        ...
        # 這裡我們只測試流的創建，不實際運行
        try:
            socket_stream = streaming_spark_session \
                .readStream \
                .format("socket") \
...
class TestStreamingTransformations:
    ...
    def streaming_spark(self):
        ...
        spark = SparkSession.builder \
            .appName("StreamingTransformTest") \
            .master("local[2]") \
            .config("spark.sql.streaming.checkpointLocation", tempfile.mkdtemp()) \
            .getOrCreate()
    ...
    def test_select_transformation(self, streaming_spark):
        ...
        query = selected_stream.writeStream \
            .outputMode("append") \
            .format("memory") \
    ...
    def test_group_by_aggregation(self, streaming_spark):
        ...
        # 按值的奇偶性分組
        aggregated_stream = rate_stream \
            .withColumn("parity", when(col("value") % 2 == 0, "even").otherwise("odd")) \
            .groupBy("parity") \
            .count()
...
class TestStatefulOperations:
    ...
    def test_update_mode_aggregation(self, stateful_spark):
        ...
        # 狀態聚合
        stateful_stream = rate_stream \
            .withColumn("mod_value", col("value") % 5) \
            .groupBy("mod_value") \
            .agg(
                count("*").alias("count"),
                avg("value").alias("avg_value")
        ...
        query = stateful_stream.writeStream \
            .outputMode("update") \
            .format("memory") \
...
class TestFileStreamOperations:
    ...
    def test_json_file_stream(self, file_spark, temp_dir):
        ...
        # 創建 JSON 流
        json_stream = file_spark \
            .readStream \
            .schema(schema) \
            .json(input_dir)
        
        # 過濾和轉換
        processed_stream = json_stream \
            .filter(col("score") > 80) \
            .select(
                col("id"),
                col("name"),
...
class TestStreamingOutputModes:
    ...
    def test_update_mode(self, output_spark):
        ...
        # 聚合操作（適合 update 模式）
        aggregated_stream = rate_stream \
            .withColumn("group_key", col("value") % 2) \
            .groupBy("group_key") \
            .agg(
                count("*").alias("count"),
                avg("value").alias("avg_value")
...
class TestStreamingErrorHandling:
    ...
    def test_query_exception_handling(self, error_spark):
        ...
        # 正常的查詢
        normal_query = rate_stream.writeStream \
            .outputMode("append") \
            .format("memory") \
    ...
    def test_invalid_output_mode(self, error_spark):
        ...
        try:
            invalid_query = rate_stream.writeStream \
                .outputMode("complete") \
                .format("memory") \
