...
from pyspark.sql.functions import col, when, lit, regexp_replace, split, concat_ws
...
3,Charlie,35,70000"""
        ...
        # 讀取 CSV
        df = spark_session.read.option("header", "true").option("inferSchema", "true").csv(csv_file)
    ...
    def test_multi_format_operations(self, spark_session, temp_dir):
        ...
        df_csv = spark_session.read.option("header", "true").option("inferSchema", "true").csv(csv_path)
...
class TestDataCleaning:
    ...
    def test_null_handling(self, spark_session):
        ...
        # 替換空字串
        df_replaced = df.withColumn("name", 
                                   when(col("name") == "", "Empty")
                                   .otherwise(col("name")))
    ...
    def test_text_cleaning(self, spark_session):
        ...
        # 清理空格
        df_trimmed = df.withColumn("name", regexp_replace(col("name"), "^\\s+|\\s+$", ""))
        ...
        # 移除特殊字符
        df_cleaned = df.withColumn("name", regexp_replace(col("name"), "[^a-zA-Z\\s]", ""))
...
class TestComplexTransformations:
    ...
    def test_string_operations(self, spark_session):
        ...
        # 提取郵件用戶名
        df_username = df.withColumn("username", 
                                   regexp_extract(col("email"), "([^@]+)", 1))
        usernames = [row.username for row in df_username.collect()]
        assert "alice.smith" in usernames
        
        # 構建全名
        df_reconstructed = df_split.withColumn("reconstructed_name",
                                             concat_ws(" ", col("first_name"), col("last_name")))
        
        reconstructed_names = [row.reconstructed_name for row in df_reconstructed.collect()]
    ...
    def test_conditional_transformations(self, spark_session):
        ...
        df = spark_session.createDataFrame(data, ["id", "name", "age", "salary", "department"])
