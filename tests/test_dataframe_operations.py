...
from pyspark.sql.functions import col, when, lit, upper, lower, avg, sum as spark_sum, count, max as spark_max
...
class TestDataFrameOperations:
    ...
    def test_dataframe_select_operations(self, sample_data):
        ...
        # 帶表達式的選擇
        df3 = sample_data.select(col("name"), (col("salary") / 12).alias("monthly_salary"))
    ...
    def test_dataframe_with_column_operations(self, sample_data):
        ...
        # 條件列
        df3 = sample_data.withColumn("level", 
    ...
    def test_dataframe_cast_operations(self, sample_data):
        ...
        # 檢查數據類型
        age_str_field = [field for field in df.schema.fields if field.name == "age_str"][0]
    ...
    def test_dataframe_when_otherwise(self, sample_data):
        ...
        df = sample_data.withColumn("salary_category",
...
class TestDataFrameIO:
    ...
    def test_write_csv(self, sample_data, temp_dir):
        ...
        sample_data.coalesce(1).write.mode("overwrite").option("header", "true").csv(output_path)
