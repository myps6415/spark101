...
from pyspark.sql.functions import (
    ...
    regexp_extract, split, size, explode, window, current_timestamp, 
    to_timestamp, date_format, desc, asc, row_number
)
...
class TestLogAnalyzer:
    ...
    def test_log_parsing(self, spark_session, sample_log_data):
        ...
        parsed_logs = logs_df.select(
            ...
            regexp_extract(col("value"), r'"[^"]*" (\d+)', 1).cast("int").alias("status"),
            regexp_extract(col("value"), r'"[^"]*" \d+ (\d+)', 1).cast("int").alias("size")
        ).filter(col("ip") != "")  # 過濾解析失敗的行
    ...
    def test_traffic_analysis(self, spark_session, sample_log_data):
        ...
        # IP訪問統計
        ip_stats = parsed_logs.groupBy("ip").agg(
        ...
        assert 401 in status_codes or 403 in status_codes or 500 in status_codes  # 錯誤請求
    ...
    def test_peak_hour_analysis(self, spark_session, temp_dir):
        ...
        time_parsed = parsed_logs.withColumn(
            "hour", 
            regexp_extract(col("timestamp_str"), r':(\d{2}):', 1).cast("int")
        )
...
class TestRealtimeETL:
    ...
    def test_data_validation(self, spark_session):
        ...
        df = spark_session.createDataFrame(raw_data, ["id", "name", "age", "email", "salary"])
    ...
    def test_data_transformation(self, spark_session):
        ...
        df = spark_session.createDataFrame(raw_data, ["full_name", "age_str", "department", "salary_str"])
        ...
        transformed_df = df.select(
            ...
            "first_name", 
            when(col("first_name").rlike("^[A-Z]+$"), 
                 regexp_replace(col("first_name"), "^(.)(.*)", 
        ...
        formatted_df = transformed_df.select(
            regexp_replace(col("first_name"), "^(.)", lambda m: m.group(1).upper()).alias("first_name"),
            regexp_replace(col("last_name"), "^(.)", lambda m: m.group(1).upper()).alias("last_name"),
            col("age"),
            col("department"),
            col("salary")
        )
    ...
    def test_incremental_processing(self, spark_session, temp_dir):
        ...
        batch1_df = spark_session.createDataFrame(batch1_data, ["user_id", "name", "date", "amount"])
        ...
        batch2_df = spark_session.createDataFrame(batch2_data, ["user_id", "name", "date", "amount"])
    ...
    def test_data_quality_monitoring(self, spark_session):
        ...
        df = spark_session.createDataFrame(data, ["id", "name", "age", "email", "salary", "date"])
...
class TestEcommerceAnalytics:
    ...
    def ecommerce_data(self, spark_session):
        ...
        users_df = spark_session.createDataFrame(users_data, ["user_id", "name", "email", "tier", "registration_date"])
        ...
        products_df = spark_session.createDataFrame(products_data, ["product_id", "product_name", "category", "price"])
        ...
        orders_df = spark_session.createDataFrame(orders_data, ["order_id", "user_id", "order_date", "total_amount"])
    ...
    def test_product_recommendation(self, ecommerce_data):
        ...
        # 假設每個訂單包含一個主要產品
        order_details = [
            (1001, 101), (1001, 104),  # Alice: Laptop + Headphones
            (1002, 102),               # Bob: Phone
            (1003, 103), (1003, 104),  # Alice: Book + Headphones
            (1004, 101),               # Charlie: Laptop
            (1005, 103),               # Diana: Book
            (1006, 101), (1006, 105),  # Eve: Laptop + Tablet
            (1007, 104)                # Bob: Headphones
        ]
        
        order_details_df = spark_session.createDataFrame(order_details, ["order_id", "product_id"])
        
        # 找出經常一起購買的產品
        # 連接訂單詳情以找到同一訂單中的產品組合
        product_pairs = order_details_df.alias("od1").join(
            order_details_df.alias("od2"),
            col("od1.order_id") == col("od2.order_id")
        ).filter(
            col("od1.product_id") < col("od2.product_id")  # 避免重複對
        ).select(
    ...
    def test_sales_performance(self, ecommerce_data):
        ...
        # 用戶層級分析
        user_performance = orders_df.join(users_df, "user_id").groupBy("tier").agg(
    ...
    def test_cohort_analysis(self, ecommerce_data):
        ...
        # 計算轉換率（註冊到首次購買）
        conversion_stats = cohort_analysis.groupBy("registration_month").agg(
            count("*").alias("registered_users"),
            spark_sum(when(col("first_order_date").isNotNull(), 1).otherwise(0)).alias("converted_users")
        ).withColumn(
            "conversion_rate",
            col("converted_users") / col("registered_users")
        )
...
class TestFinancialRiskManagement:
    ...
    def test_fraud_detection(self, financial_data):
        ...
        suspicious_transactions = transactions_df.withColumn(
            "hour", 
            date_format(col("timestamp"), "HH").cast("int")
        ).withColumn(
            "is_suspicious",
        ...
        # 按賬戶統計風險
        account_risk = suspicious_transactions.groupBy("account_id").agg(
            count("*").alias("total_transactions"),
            spark_sum(when(col("is_suspicious") != "Normal", 1).otherwise(0)).alias("suspicious_count")
        ).withColumn(
            "risk_ratio",
            col("suspicious_count") / col("total_transactions")
        )
    ...
    def test_transaction_monitoring(self, financial_data):
        ...
        transaction_patterns = enriched_transactions.withColumn(
            ...
            when(col("prev_transaction_time").isNotNull(),
                 (col("timestamp").cast("long") - col("prev_transaction_time").cast("long"))
            ).otherwise(None)
        )
        ...
        # 每日交易限制檢查
        daily_limits = enriched_transactions.withColumn(
            "date", 
            date_format(col("timestamp"), "yyyy-MM-dd")
        ).groupBy("account_id", "date").agg(
        ...
        exceeded_limits = daily_limits.filter(
            (col("daily_transaction_count") > 10) | 
            (col("daily_total_amount") > 10000)
        )
    ...
    def test_credit_risk_assessment(self, financial_data):
        ...
        account_metrics = transactions_df.groupBy("account_id").agg(
            ...
            spark_sum(when(col("type") == "Transfer", col("amount")).otherwise(0)).alias("total_transfers")
        )
        ...
        risk_scored = risk_assessment.withColumn(
            "utilization_ratio",
            col("total_transaction_amount") / col("balance")
        ).withColumn(
            "risk_score",
    ...
    def test_portfolio_analysis(self, financial_data):
        ...
        portfolio_df = spark_session.createDataFrame(
            portfolio_data, 
            ["account_id", "symbol", "shares", "current_price"]
        )
        
        # 計算投資組合價值
        portfolio_value = portfolio_df.withColumn(
            "position_value",
            col("shares") * col("current_price")
        )
        ...
        investment_ratio = portfolio_analysis.withColumn(
            "investment_to_balance_ratio",
            col("total_portfolio_value") / col("balance")
        )
        
        # 檢查組合分析結果
        assert investment_ratio.count() == portfolio_df.select("account_id").distinct().count()
        
        # 檢查風險分散度
        concentration_risk = investment_ratio.withColumn(
            "concentration_risk",
            col("largest_position") / col("total_portfolio_value")
        )
