...
from pyspark.sql.functions import (
    col, when, lit, avg, sum as spark_sum, count, max as spark_max, min as spark_min,
    rank, dense_rank, row_number, lag, lead, first, last,
    regexp_extract, regexp_replace, split, concat, concat_ws,
    year, month, dayofmonth, date_add, date_sub, datediff,
...
class TestSparkSQL:
    ...
    def test_basic_sql_queries(self, spark_session, sample_data):
        ...
        # 聚合查詢
        result = spark_session.sql("SELECT AVG(age) as avg_age, MAX(salary) as max_salary FROM employees")
        row = result.collect()[0]
        assert row.avg_age == 29.5  # (25+30+35+28)/4
        assert row.max_salary == 85000
        
        # GROUP BY 查詢
        result = spark_session.sql("SELECT job, COUNT(*) as count FROM employees GROUP BY job")
    ...
    def test_join_operations_sql(self, spark_session):
        ...
        employees_df = spark_session.createDataFrame(employees_data, ["id", "name", "department"])
        ...
        departments_df = spark_session.createDataFrame(departments_data, ["dept_name", "location"])
