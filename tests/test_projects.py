...
from pyspark.sql.functions import col, when, lit, count, sum as spark_sum, avg, desc
...
class TestLogAnalyzerProject:
    ...
    def test_log_parsing_comprehensive(self, spark_session, log_analyzer_data):
        ...
        parsed_logs = raw_logs.select(
            ...
            regexp_extract(col("value"), r'"[^"]*" (\d+)', 1).cast("int").alias("status_code"),
            regexp_extract(col("value"), r'"[^"]*" \d+ (\d+)', 1).cast("int").alias("response_size"),
    ...
    def test_traffic_analysis_detailed(self, spark_session, log_analyzer_data):
        """測試詳細的流量分析"""
        parsed_logs = self.test_log_parsing_comprehensive(spark_session, log_analyzer_data)
        
        # IP地址分析
        ip_analysis = parsed_logs.groupBy("ip_address").agg(
        ...
        # URL分析
        url_analysis = parsed_logs.groupBy("url").agg(
        ...
        # 狀態碼分析
        status_analysis = parsed_logs.groupBy("status_code").count().orderBy("status_code")
    ...
    def test_security_analysis(self, spark_session, log_analyzer_data):
        ...
        security_analysis = parsed_logs.withColumn(
            ...
            .when(col("user_agent").rlike("(?i).*(bot|crawler|scanner).*"), "Bot Activity")
            .otherwise("Normal")
        )
        
        # 威脅統計
        threat_stats = security_analysis.groupBy("threat_type").count().orderBy(desc("count"))
        ...
        # 按IP檢測可疑活動
        suspicious_ips = security_analysis.filter(col("threat_type") != "Normal") \
                                         .groupBy("ip_address") \
...
class TestMonitoringSystemProject:
    ...
    def monitoring_data(self, spark_session):
        ...
        return spark_session.createDataFrame(
            metrics_data,
            ["server_id", "timestamp", "metric_name", "metric_value"]
        )
    ...
    def test_threshold_monitoring(self, monitoring_data):
        ...
        # 檢測超過閾值的指標
        alerts = monitoring_data.withColumn(
            "threshold",
            when(col("metric_name") == "cpu_usage", lit(thresholds["cpu_usage"]))
            .when(col("metric_name") == "memory_usage", lit(thresholds["memory_usage"]))
            .when(col("metric_name") == "disk_usage", lit(thresholds["disk_usage"]))
            .otherwise(lit(100.0))
        ).withColumn(
            "is_alert",
            col("metric_value") > col("threshold")
    ...
    def test_trend_analysis(self, monitoring_data):
        ...
        # 找出高使用率的服務器
        high_usage_servers = avg_metrics.filter(col("avg_value") > 80).groupBy("server_id").count()
    ...
    def test_anomaly_detection(self, monitoring_data):
        """測試異常檢測"""
        # 計算每個指標的統計信息
        metric_stats = monitoring_data.groupBy("metric_name").agg(
            avg("metric_value").alias("mean_value"),
            spark_sum((col("metric_value") - avg("metric_value")) ** 2).alias("variance_sum"),
            count("*").alias("count")
        ).withColumn(
            "std_dev",
            (col("variance_sum") / col("count")) ** 0.5
        )
        ...
        anomalies = monitoring_data.withColumn(
            ...
            .when((col("metric_name") == "memory_usage") & (col("metric_value") > 95), True)
            .when((col("metric_name") == "disk_usage") & (col("metric_value") > 95), True)
            .otherwise(False)
    ...
    def test_health_score_calculation(self, monitoring_data):
        """測試健康分數計算"""
        # 計算每個服務器的健康分數
        health_scores = monitoring_data.groupBy("server_id").agg(
            avg(when(col("metric_name") == "cpu_usage", col("metric_value"))).alias("avg_cpu"),
            avg(when(col("metric_name") == "memory_usage", col("metric_value"))).alias("avg_memory"),
            avg(when(col("metric_name") == "disk_usage", col("metric_value"))).alias("avg_disk")
        ).withColumn(
            "health_score",
...
class TestRecommendationSystemProject:
    ...
    def test_user_similarity(self, recommendation_data):
        ...
        # 創建用戶-物品矩陣（簡化版本）
        user_item_matrix = ratings_df.groupBy("user_id").pivot("item_id").agg(avg("rating"))
        
        # 檢查用戶-物品矩陣
        assert user_item_matrix.count() == 5  # 5個用戶
        
        # 計算用戶共同評分的物品
        user_pairs = ratings_df.alias("r1").join(
            ratings_df.alias("r2"),
            (col("r1.item_id") == col("r2.item_id")) & (col("r1.user_id") < col("r2.user_id"))
        ).select(
        ...
        # 計算用戶對的共同評分數
        user_similarity = user_pairs.groupBy("user1", "user2").agg(
            count("*").alias("common_items"),
            avg(abs(col("rating1") - col("rating2"))).alias("avg_rating_diff")
        ).withColumn(
            "similarity_score",
            col("common_items") / (1 + col("avg_rating_diff"))
        )
    ...
    def test_item_based_recommendations(self, recommendation_data):
        ...
        # 物品共現分析
        item_cooccurrence = ratings_df.alias("r1").join(
            ratings_df.alias("r2"),
            (col("r1.user_id") == col("r2.user_id")) & (col("r1.item_id") < col("r2.item_id"))
        ).select(
        ...
        if item_similarity.count() > 0:
            ...
            # 為特定物品生成推薦
            target_item = 101
            similar_items = item_similarity.filter(
                (col("item1") == target_item) | (col("item2") == target_item)
            ).withColumn(
                "recommended_item",
                when(col("item1") == target_item, col("item2")).otherwise(col("item1"))
            ).select("recommended_item", "cooccurrence_count").orderBy(desc("cooccurrence_count"))
    ...
    def test_popularity_based_recommendations(self, recommendation_data):
        ...
        # 加入物品信息
        popular_items = item_popularity.join(items_df, "item_id") \
        ...
        # 按類型分析流行度
        genre_popularity = popular_items.groupBy("genre").agg(
    ...
    def test_recommendation_evaluation(self, recommendation_data):
        ...
        predicted_ratings = actual_ratings.withColumn("predicted_rating", lit(global_avg))
        
        # 計算評估指標
        evaluation_metrics = predicted_ratings.withColumn(
            "absolute_error",
            abs(col("actual_avg_rating") - col("predicted_rating"))
        ).agg(
            avg("absolute_error").alias("mae"),  # Mean Absolute Error
            spark_sum((col("actual_avg_rating") - col("predicted_rating")) ** 2).alias("sse")
        )
...
class TestProjectIntegration:
    ...
    def test_cross_project_data_sharing(self, spark_session, temp_dir):
        ...
        events_df = spark_session.createDataFrame(
            shared_data,
            ["user_id", "timestamp", "event_type", "platform"]
        )
        ...
        # 檢查數據一致性
        total_events_from_users = user_sessions.agg(spark_sum("event_count")).collect()[0][0]
        total_events_from_platforms = platform_analysis.agg(spark_sum("total_events")).collect()[0][0]
    ...
    def test_project_performance_comparison(self, spark_session):
        ...
        for result in performance_results:
            ...
            assert result["large_result"] >= result["small_result"]  # 大數據集結果應該>=小數據集
    ...
    def test_error_handling_across_projects(self, spark_session):
        ...
        try:
            # 創建可能超出內存的操作
            memory_intensive_data = [(i, "x" * 1000, [j for j in range(100)]) for i in range(100)]
            memory_df = spark_session.createDataFrame(memory_intensive_data, ["id", "text", "numbers"])
...
<parameter name="todos">[{"id": "1", "content": "Review existing test files to understand current test coverage", "status": "completed", "priority": "high"}, {"id": "2", "content": "Identify missing test coverage for chapters 3-8", "status": "completed", "priority": "high"}, {"id": "3", "content": "Create test_chapter03.py for schema operations", "status": "completed", "priority": "high"}, {"id": "4", "content": "Create test_chapter04.py for Spark SQL", "status": "completed", "priority": "high"}, {"id": "5", "content": "Create test_chapter05.py for Streaming", "status": "completed", "priority": "high"}, {"id": "6", "content": "Create test_chapter06.py for MLlib", "status": "completed", "priority": "high"}, {"id": "7", "content": "Create test_chapter07.py for Performance Tuning", "status": "completed", "priority": "high"}, {"id": "8", "content": "Create test_chapter08.py for Projects", "status": "completed", "priority": "high"}, {"id": "9", "content": "Create test_projects.py for project-specific tests", "status": "completed", "priority": "medium"}]