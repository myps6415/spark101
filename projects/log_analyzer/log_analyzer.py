...
class LogAnalyzer:
    ...
    def parse_apache_log(self, log_file_path: str) -> DataFrame:
        ...
        parsed_logs = raw_logs.select(
            ...
            regexp_extract(col("value"), log_pattern, 7).cast("int").alias("response_size"),
            regexp_extract(col("value"), log_pattern, 8).alias("referer"),
            regexp_extract(col("value"), log_pattern, 9).alias("user_agent"),
            regexp_extract(col("value"), log_pattern, 10).cast("double").alias("response_time"),
    ...
    def detect_security_threats(self) -> Dict[str, DataFrame]:
        ...
        threats['sql_injection'] = self.log_df.filter(sql_injection_condition) \
            .groupBy("ip", "path") \
            .agg(
        ...
        threats['directory_traversal'] = self.log_df.filter(directory_traversal_condition) \
            .groupBy("ip", "path") \
            .agg(
                count("*").alias("attempt_count"),
                collect_list("timestamp").alias("timestamps")
        ...
        brute_force_condition = (
            (col("path").rlike(r"(?i)(login|admin|signin)")) &
            (col("status_code").isin([401, 403]))
        )
        threats['brute_force'] = self.log_df.filter(brute_force_condition) \
            .groupBy("ip") \
            .agg(
        ...
        # 4. 異常高頻訪問檢測
        high_freq_condition = self.log_df.groupBy("ip") \
        ...
        # 5. 敏感路徑訪問檢測
        sensitive_path_condition = col("path").rlike("|".join(self.config['sensitive_paths']))
        threats['sensitive_access'] = self.log_df.filter(sensitive_path_condition) \
            .groupBy("ip", "path") \
            .agg(
        ...
        threats['suspicious_bots'] = self.log_df.filter(bot_condition) \
    ...
    def analyze_traffic_patterns(self) -> Dict[str, DataFrame]:
        ...
        # 1. 時間序列分析
        patterns['hourly_traffic'] = self.log_df \
        ...
        # 2. 每日流量模式
        patterns['daily_traffic'] = self.log_df \
        ...
        # 3. 熱門路徑分析
        patterns['popular_paths'] = self.log_df \
            .groupBy("path") \
            .agg(
                count("*").alias("request_count"),
                avg("response_time").alias("avg_response_time"),
        ...
        # 4. 狀態碼分析
        patterns['status_analysis'] = self.log_df \
            .groupBy("status_code") \
            .agg(
                count("*").alias("count"),
        ...
        # 5. 用戶代理分析
        patterns['user_agent_analysis'] = self.log_df \
            .groupBy("user_agent") \
            .agg(
                count("*").alias("request_count"),
                countDistinct("ip").alias("unique_ips")
        ...
        # 6. 響應時間分析
        patterns['response_time_analysis'] = self.log_df \
            .select(
                "path",
                "response_time",
                ...
                .otherwise("very_slow").alias("response_category")
            ) \
    ...
    def detect_anomalies(self) -> Dict[str, DataFrame]:
        ...
        anomalies['slow_requests'] = self.log_df \
            .filter(col("response_time") > threshold) \
            .select("timestamp", "ip", "path", "response_time", "status_code") \
            .orderBy(col("response_time").desc())
        
        # 2. 請求頻率異常
        window_spec = Window.partitionBy("ip").orderBy("timestamp").rangeBetween(-300, 0)  # 5分鐘窗口
        
        request_frequency = self.log_df \
            .withColumn("requests_in_window", count("*").over(window_spec)) \
            .filter(col("requests_in_window") > 100)  # 5分鐘內超過100次請求
        
        anomalies['high_frequency_requests'] = request_frequency \
            .select("timestamp", "ip", "path", "requests_in_window") \
            .orderBy(col("requests_in_window").desc())
        
        # 3. 狀態碼異常模式
        error_patterns = self.log_df \
        ...
        # 4. 路徑訪問異常
        path_anomalies = self.log_df \
            .groupBy("ip", "path") \
            .agg(count("*").alias("access_count")) \
            .filter(col("access_count") > 200)  # 單個IP對單個路徑的訪問超過200次
    ...
    def generate_security_report(self, output_path: str) -> None:
        ...
        if report["threats"]["sql_injection"]["count"] > 0:
            recommendations.append("檢測到 SQL 注入嘗試，建議加強輸入驗證和使用參數化查詢")
    ...
    def generate_visualizations(self, output_dir: str) -> None:
        ...
        # 每小時錯誤數
        axes[1, 1].plot(hourly_data['hour'], hourly_data['error_count'], marker='o', color='orange')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'hourly_traffic_analysis.png'), dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.pie(status_data['count'], labels=status_data['status_code'], autopct='%1.1f%%')
        plt.title('HTTP 狀態碼分佈')
        plt.savefig(os.path.join(output_dir, 'status_code_distribution.png'), dpi=300, bbox_inches='tight')
        plt.close()
        ...
        plt.ylabel('路徑')
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, 'popular_paths.png'), dpi=300, bbox_inches='tight')
        plt.close()
