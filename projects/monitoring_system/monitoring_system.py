...
class MetricsCollector:
    ...
    def load_metrics(self, input_path: str) -> DataFrame:
        ...
        try:
            ...
            df = df.withColumn(
                "timestamp",
                when(col("timestamp").isNotNull(), col("timestamp")).otherwise(
                    current_timestamp()
                ),
            )
...
class AlertManager:
    ...
    def check_thresholds(self, metrics_df: DataFrame) -> DataFrame:
        ...
        # CPU 使用率警報
        cpu_alerts = (
            metrics_df.filter(col("cpu_usage") > self.config.cpu_threshold)
        ...
        # 記憶體使用率警報
        memory_alerts = (
            metrics_df.filter(col("memory_usage") > self.config.memory_threshold)
        ...
        # 磁碟使用率警報
        disk_alerts = (
            metrics_df.filter(col("disk_usage") > self.config.disk_threshold)
        ...
        # 錯誤率警報
        error_alerts = (
            metrics_df.filter(col("error_rate") > self.config.error_rate_threshold)
        ...
        # 合併所有警報
        all_alerts = (
            cpu_alerts.union(memory_alerts)
            .union(disk_alerts)
...
class AnomalyDetector:
    ...
    def detect_statistical_anomalies(self, metrics_df: DataFrame) -> DataFrame:
        ...
        # 計算移動平均和標準差
        window_spec = (
            Window.partitionBy("server_id", "service_name")
            .orderBy("timestamp")
            .rowsBetween(-5, 0)
        )

        with_stats = (
            metrics_df.withColumn("cpu_moving_avg", avg("cpu_usage").over(window_spec))
            .withColumn("cpu_moving_std", stddev("cpu_usage").over(window_spec))
            .withColumn("memory_moving_avg", avg("memory_usage").over(window_spec))
            .withColumn("memory_moving_std", stddev("memory_usage").over(window_spec))
            .withColumn(
        ...
        anomalies = (
            with_stats.filter(
                (
                    abs(col("cpu_usage") - col("cpu_moving_avg"))
                    > 2 * col("cpu_moving_std")
                )
                | (
                    abs(col("memory_usage") - col("memory_moving_avg"))
                    > 2 * col("memory_moving_std")
                )
                | (
                    abs(col("response_time") - col("response_time_moving_avg"))
                    > 2 * col("response_time_moving_std")
                )
            )
            ...
            .withColumn(
                ...
                greatest(
                    ...
                    / col("cpu_moving_std"),
                    abs(col("memory_usage") - col("memory_moving_avg"))
                    / col("memory_moving_std"),
                    abs(col("response_time") - col("response_time_moving_avg"))
                    / col("response_time_moving_std"),
                ),
            )
        )
    ...
    def detect_ml_anomalies(self, metrics_df: DataFrame) -> DataFrame:
        ...
        try:
            ...
            anomalies = anomalies.withColumn(
                "anomaly_type", lit("ml_clustering")
    ...
    def detect_pattern_anomalies(self, metrics_df: DataFrame) -> DataFrame:
        ...
        anomaly_count = metrics_df.withColumn(
            "anomaly_count",
            sum(
                [when(condition, 1).otherwise(0) for condition in threshold_conditions]
            ),
        )

        # 多個指標同時異常視為模式異常
        pattern_anomalies = (
            anomaly_count.filter(col("anomaly_count") >= 3)
...
class MonitoringSystem:
    ...
    def run_monitoring_cycle(self) -> Dict[str, Any]:
        ...
        if self.config.enable_ml_detection:
            anomalies["statistical"] = (
                self.anomaly_detector.detect_statistical_anomalies(metrics_df)
            )
            anomalies["ml"] = self.anomaly_detector.detect_ml_anomalies(metrics_df)
            anomalies["pattern"] = self.anomaly_detector.detect_pattern_anomalies(
                metrics_df
            )
    ...
    def analyze_trends(self, metrics_df: DataFrame) -> Dict[str, Any]:
        ...
        # 按小時分析趨勢
        hourly_trends = (
            metrics_df.withColumn("hour", hour("timestamp"))
            .groupBy("hour")
            .agg(
                avg("cpu_usage").alias("avg_cpu"),
                avg("memory_usage").alias("avg_memory"),
    ...
    def save_report(self, report: Dict[str, Any]) -> None:
        ...
        report_path = os.path.join(
            self.config.output_path, f"monitoring_report_{timestamp}.json"
        )
    ...
    def generate_dashboard(self, metrics_df: DataFrame) -> None:
        ...
        # 按服務分組分析
        service_metrics = (
            metrics_df.groupBy("service_name")
            .agg(
                avg("cpu_usage").alias("avg_cpu"),
                avg("memory_usage").alias("avg_memory"),
                avg("disk_usage").alias("avg_disk"),
        ...
        axes[1, 2].plot(
            hourly_metrics["hour"],
            hourly_metrics["avg_response_time"],
            marker="o",
            color="green",
        )
        ...
        dashboard_path = os.path.join(
            self.config.output_path, "monitoring_dashboard.png"
        )
    ...
    def run_continuous_monitoring(self) -> None:
        ...
        try:
            while True:
                ...
                # 生成儀表板
                if os.path.exists(self.config.input_path):
                    metrics_df = self.metrics_collector.load_metrics(
...
def load_config(config_path: str) -> MonitoringConfig:
    ...
    if os.path.exists(config_path):
        ...
        # 使用預設配置
        return MonitoringConfig(
            input_path="", output_path="./monitoring_output", alert_config=AlertConfig()
        )
