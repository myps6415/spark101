...
from pyspark.sql.functions import (avg, col, collect_list, count,
...
class RealTimeETL:
            ...
            for i in range(duration_seconds):
                ...
                for j in range(random.randint(3, 8)):
                    event_type = random.choice(
                        ["order", "product_view", "user_signup", "cart_add"]
                    )
    ...
    def data_transformation(self, df):
        ...
        transformed_df = df.withColumn(
            "is_purchase", when(col("event_type") == "order", True).otherwise(False)
        )

        # 2. 添加時間維度
        transformed_df = transformed_df.withColumn(
            "hour", date_format(col("event_timestamp"), "HH")
        ).withColumn("date", date_format(col("event_timestamp"), "yyyy-MM-dd"))

        # 3. 計算衍生指標
        transformed_df = transformed_df.withColumn(
            "revenue", when(col("event_type") == "order", col("amount")).otherwise(0)
        )
    ...
    def data_enrichment(self, df):
        ...
        # 2. 聯結維度表
        enriched_df = df.join(user_dim, "user_id", "left").join(
            product_dim, "product_id", "left"
        )

        # 3. 添加計算列
        enriched_df = enriched_df.withColumn(
            "is_high_value", when(col("price_tier") == "High", True).otherwise(False)
        )
    ...
    def create_aggregations(self, df):
        ...
        # 1. 實時銷售指標
        sales_metrics = (
            df.filter(col("event_type") == "order")
        ...
        # 3. 用戶行為分析
        user_behavior = (
            df.withWatermark("event_timestamp", "1 minute")
            .groupBy(
    ...
    def quality_checks(self, df):
        ...
        range_check = df.select(
            spark_sum(when(col("amount") < 0, 1).otherwise(0)).alias(
                "negative_amounts"
            ),
            spark_sum(when(col("quantity") < 0, 1).otherwise(0)).alias(
                "negative_quantities"
            ),
            spark_sum(when(col("user_id") <= 0, 1).otherwise(0)).alias(
                "invalid_user_ids"
            ),
        )
    ...
    def setup_outputs(self, sales_metrics, product_metrics, user_behavior):
        ...
        # 2. 文件輸出
        file_query = (
            product_metrics.writeStream.outputMode("append")
            .format("json")
            .option("path", os.path.join(self.output_dir, "product_metrics"))
            .option(
                "checkpointLocation",
                os.path.join(self.checkpoint_dir, "product_checkpoint"),
            )
        ...
        # 3. 內存輸出（用於查詢）
        memory_query = (
            user_behavior.writeStream.outputMode("append")
            .format("memory")
    ...
    def monitoring_and_alerting(self, df):
        ...
        health_check = (
            ...
            .agg(
                ...
                spark_sum(when(col("event_type").isNull(), 1).otherwise(0)).alias(
                    "malformed_events"
                ),
            )
            .withColumn(
    ...
    def run_etl_pipeline(self):
        ...
        # 6. 創建聚合視圖
        sales_metrics, product_metrics, user_behavior = self.create_aggregations(
            enriched_df
        )

        # 7. 設置輸出
        console_query, file_query, memory_query = self.setup_outputs(
            sales_metrics, product_metrics, user_behavior
        )

        # 8. 監控和警報
        anomalies, business_metrics, health_check = self.monitoring_and_alerting(
            enriched_df
        )

        # 9. 監控查詢
        monitoring_query = (
            health_check.writeStream.outputMode("append")
            .format("console")
            .option("truncate", "false")
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("Real-time ETL Pipeline")
        .master("local[*]")
    ...
    try:
        ...
        for query in active_queries:
            if query.lastProgress:
                batch_duration = query.lastProgress.get("durationMs", {}).get(
                    "triggerExecution", 0
                )
