...
from pyspark.sql.functions import (asc, avg, col, collect_list, count,
                                   current_timestamp, date_format, date_sub,
                                   dayofweek, desc, hour, isnan, isnull, lit)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import (monotonically_increasing_id, regexp_extract,
                                   size, split)
...
class LogAnalyzer:
    ...
    def parse_logs(self, log_file):
        ...
        # 添加時間維度
        parsed_logs = (
            parsed_logs.withColumn("hour", hour(col("timestamp")))
    ...
    def data_quality_check(self, df):
        ...
        null_counts = (
            ...
            .agg(*[spark_sum(col(c)).alias(f"{c}_nulls") for c in df.columns])
            .collect()[0]
        )
        ...
        for col_name in df.columns:
            ...
            if null_count > 0:
                print(
                    f"  {col_name}: {null_count} ({null_count/total_records*100:.2f}%)"
                )
    ...
    def basic_analysis(self, df):
        ...
        # 每日訪問量
        daily_requests = (
            df.groupBy("date").agg(count("*").alias("requests")).orderBy("date")
        )
        ...
        # 每小時訪問量
        hourly_requests = (
            df.groupBy("hour").agg(count("*").alias("requests")).orderBy("hour")
        )
        ...
        # 最活躍用戶
        top_users = (
            df.groupBy("ip").agg(count("*").alias("requests")).orderBy(desc("requests"))
        )
    ...
    def advanced_analysis(self, df):
        ...
        # 大請求分析
        large_requests = (
            df.filter(col("size") > 5000)
            .groupBy("path")
            .agg(count("*").alias("large_request_count"), avg("size").alias("avg_size"))
    ...
    def real_time_monitoring(self, df):
        ...
        metrics = df.agg(
            ...
            (
                spark_sum(when(col("status") >= 400, 1).otherwise(0))
                * 100.0
                / count("*")
            ).alias("error_rate"),
            (
                spark_sum(when(col("status") >= 500, 1).otherwise(0))
                * 100.0
                / count("*")
            ).alias("server_error_rate"),
        ...
        high_error_pages = (
            ...
            .withColumn(
                "error_rate", col("error_requests") * 100.0 / col("total_requests")
            )
        ...
        attack_patterns = (
            df.filter(
                col("path").rlike(
                    r".*(\.\.|script|sql|union|select|drop|delete|insert|update).*"
                )
    ...
    def create_dashboard_data(self, df):
        ...
        # 導出各種格式的數據
        time_series.coalesce(1).write.mode("overwrite").option("header", "true").csv(
            f"{dashboard_path}/time_series"
        )
        overview.coalesce(1).write.mode("overwrite").option("header", "true").csv(
            f"{dashboard_path}/overview"
        )
    ...
    def generate_report(self, df):
        ...
        report = {
            ...
            "top_pages": [
                {"path": row["path"], "requests": row["requests"]} for row in top_pages
            ],
            "recommendations": [
                "監控錯誤率較高的頁面",
                "優化響應時間較長的請求",
        ...
        print("\n報告摘要:")
        print(
            f"  分析期間: {report['summary']['analysis_period']['start']} 至 {report['summary']['analysis_period']['end']}"
        )
        ...
        print(
            f"  熱門頁面: {', '.join([page['path'] for page in report['top_pages'][:3]])}"
        )
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("Log Analyzer")
        .master("local[*]")
