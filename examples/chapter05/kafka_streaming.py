...
def create_sample_kafka_data():
    ...
    for i in range(100):
        ...
        if event_type == "page_view":
            base_event.update(
                {"page": random.choice(pages), "duration": random.randint(10, 300)}
            )
...
def main():
    ...
    events_df = spark.createDataFrame(
        [(json.dumps(event),) for event in sample_events], ["value"]
    )
    ...
    # 按事件類型統計
    event_stats = parsed_df.groupBy("event_type").agg(
    ...
    # 用戶會話統計
    user_sessions = parsed_df.groupBy("user_id", "session_id").agg(
    ...
    windowed_df = parsed_df.withColumn("event_time", to_timestamp(col("timestamp")))

    # 10分鐘窗口的事件統計
    windowed_stats = windowed_df.groupBy(
        window(col("event_time"), "10 minutes"), col("event_type")
    ).agg(
        count("*").alias("event_count"), spark_sum("quantity").alias("total_quantity")
    )
    ...
    # 產品瀏覽統計
    product_views = (
        parsed_df.filter(col("event_type") == "page_view")
    ...
    # 地區和設備統計
    location_device_stats = (
        parsed_df.filter(col("event_type") == "user_login")
    ...
    ab_test_df = parsed_df.withColumn(
        "test_group", when(rand() > 0.5, "A").otherwise("B")
    )

    # A/B 測試轉換率
    ab_conversion = ab_test_df.groupBy("test_group").agg(
    ...
    dashboard_metrics = parsed_df.agg(
        count("*").alias("total_events"),
        spark_sum(when(col("event_type") == "user_login", 1).otherwise(0)).alias(
            "total_logins"
        ),
