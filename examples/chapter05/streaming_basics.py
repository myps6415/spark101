...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("Spark Streaming Basics")
        .master("local[*]")
    ...
    # 從文件系統讀取流數據
    streaming_df = (
        spark.readStream.format("json").schema(schema).option("path", input_dir).load()
    )
    ...
    generator_thread = threading.Thread(
        target=generate_sample_data, args=(input_dir, 15)  # 生成15秒的數據
    )
    ...
    # 按產品統計
    product_stats = streaming_df.groupBy("product").agg(
    ...
    print("開始產品統計...")
    aggregation_query = (
        product_stats.writeStream.outputMode("complete")
        .format("console")
        .option("truncate", "false")
        .trigger(processingTime="5 seconds")
        .start()
    ...
    windowed_df = streaming_df.withColumn("event_time", to_timestamp(col("timestamp")))

    # 創建時間窗口統計
    windowed_stats = (
        windowed_df.withWatermark("event_time", "1 minute")
        .groupBy(window(col("event_time"), "10 seconds", "5 seconds"), col("product"))
        .agg(count("*").alias("count"), spark_sum("quantity").alias("total_quantity"))
    )
    ...
    # 用戶總購買統計
    user_totals = streaming_df.groupBy("user_id").agg(
    ...
    print("開始用戶統計...")
    user_query = (
        user_totals.writeStream.outputMode("complete")
        .format("console")
        .option("truncate", "false")
        .trigger(processingTime="5 seconds")
        .start()
    ...
    generator_thread2 = threading.Thread(
        target=generate_sample_data, args=(input_dir, 10)
    )
    ...
    try:
        ...
        filtered_df = streaming_df.filter(
            (col("quantity") > 0) & (col("price") > 0) & (col("user_id").isNotNull())
        )
    ...
    generator_thread3 = threading.Thread(
        target=generate_sample_data, args=(input_dir, 8)
    )
    ...
    print("開始優化查詢...")
    optimized_query = (
        optimized_df.writeStream.outputMode("complete")
        .format("console")
        .option("truncate", "false")
    ...
    generator_thread4 = threading.Thread(
        target=generate_sample_data, args=(input_dir, 5)
    )
