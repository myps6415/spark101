...
from pyspark.sql.functions import (col, collect_list, explode, map_keys,
                                   map_values, struct, udf)
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("Advanced SQL Functions")
        .master("local[*]")
    ...
    # 從JSON創建DataFrame
    df = spark.read.json(
        spark.sparkContext.parallelize([json.dumps(record) for record in complex_data])
    )
    ...
    text_df = spark.createDataFrame(
        text_data, ["user_id", "full_name", "email", "contact"]
    )
