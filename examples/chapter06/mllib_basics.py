...
from pyspark.sql.functions import col, when, isnan, isnull, rand
from pyspark.ml.feature import (
    VectorAssembler, StringIndexer, OneHotEncoder, StandardScaler, 
    MinMaxScaler, PCA, HashingTF, IDF, Tokenizer, StopWordsRemover
)
...
def main():
    ...
    for i in range(1000):
        ...
        # 基於特徵生成目標值（有一定的模式）
        target = 1 if (income > 60000 and experience > 3) or (education == 2 and income > 45000) else 0
        # 添加一些隨機性
    ...
    # 創建年齡分組
    df_processed = df.withColumn("age_group", 
                                when(col("age") < 30, 0)
                                .when(col("age") < 45, 1)
                                .otherwise(2))
    
    # 創建收入分組
    df_processed = df_processed.withColumn("income_group",
                                         when(col("income") < 50000, 0)
                                         .when(col("income") < 100000, 1)
                                         .otherwise(2))
    ...
    # 選擇特徵列
    feature_cols = ["age", "income", "experience", "gender", "education", "age_group", "income_group"]
