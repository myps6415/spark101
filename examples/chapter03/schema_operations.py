...
from pyspark.sql.types import (
    ...
    DoubleType, BooleanType, DateType, TimestampType, ArrayType
)
from pyspark.sql.functions import col, to_date, to_timestamp, cast, regexp_extract
from datetime import datetime, date

def main():
    # 創建 SparkSession
    spark = SparkSession.builder \
        .appName("Schema Operations") \
        .master("local[*]") \
        .getOrCreate()
    ...
    # 準備複雜數據
    complex_data = [
        (1, "Alice", 25, 75000.0, True, date(2020, 1, 15), 
         datetime(2024, 1, 10, 9, 30, 0), ["Python", "Spark", "SQL"],
         ("123 Main St", "New York", "10001")),
        (2, "Bob", 30, 85000.0, True, date(2019, 3, 20),
         datetime(2024, 1, 9, 14, 45, 0), ["Java", "Scala", "Kafka"],
         ("456 Oak Ave", "Boston", "02101")),
        (3, "Charlie", 35, 65000.0, False, date(2021, 7, 10),
         datetime(2024, 1, 8, 11, 20, 0), ["JavaScript", "React", "Node.js"],
    ...
    string_df = spark.createDataFrame(string_data, ["id_str", "name", "age_str", "salary_str"])
    ...
    date_df = spark.createDataFrame(date_data, ["name", "hire_date_str", "last_login_str"])
    ...
    for col_name in columns_to_add:
        if col_name == "bonus":
            ...
            dynamic_df = dynamic_df.withColumn(col_name, col("salary") - col("tax") + col("bonus"))
