...
from pyspark.sql.functions import avg, broadcast, col, count, lit
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import monotonically_increasing_id, rand, randn
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("Performance Tuning")
        .master("local[*]")
    ...
    def with_cache():
        ...
        result2 = (
            df_cached.filter(col("status") == "active").agg(avg("value")).collect()
        )
        return result1, result2
    ...
    print(
        f"緩存改善: {((time_without_cache - time_with_cache) / time_without_cache * 100):.1f}%"
    )
    ...
    small_df = spark.createDataFrame(
        [("0", "Category A"), ("1", "Category B"), ("2", "Category C")], ["id", "name"]
    )
    ...
    print(
        f"廣播改善: {((time_without_broadcast - time_with_broadcast) / time_without_broadcast * 100):.1f}%"
    )
    ...
    skewed_data = (
        ...
        .withColumn(
            "key",
            when(col("id") % 100 == 0, "hot_key").otherwise(col("id").cast("string")),
        )
    ...
    small_df_for_join = spark.range(1000).withColumn(
        "join_key", col("id").cast("string")
    )

    # AQE 會自動優化這個查詢
    aqe_query = large_df.join(
        small_df_for_join, large_df.category == small_df_for_join.join_key, "left"
    )
    ...
    complex_df = df.withColumn(
        "complex_col",
        when(col("id") % 2 == 0, "even_" + col("id").cast("string")).otherwise(
            "odd_" + col("id").cast("string")
        ),
    )
    ...
    def with_pre_aggregation():
        return (
            pre_aggregated.filter(col("status") == "active")
            .agg(spark_sum("count"))
            .collect()
        )
    ...
    # 創建維度表
    dim_table = spark.range(10).withColumn(
        "status", when(col("id") % 2 == 0, "active").otherwise("inactive")
    )
    ...
    print(
        f"  - 建議執行器內存: {total_memory / len(executor_infos) / (1024**3):.2f} GB"
    )
