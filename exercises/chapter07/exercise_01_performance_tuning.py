...
def main():
    ...
    spark = (
        SparkSession.builder.appName("性能調優練習")
        .master("local[*]")
    ...
    # 創建小的維度表
    product_data = [
        (f"PROD-{i:03d}", f"Product {i}", f"Category {i % 10}") for i in range(100)
    ]
    ...
    for rdd_info in sc.statusTracker().getRddInfos():
        if rdd_info.numCachedPartitions > 0:
            print(
                f"RDD {rdd_info.id}: {rdd_info.numCachedPartitions}/{rdd_info.numPartitions} 分區已緩存"
            )
