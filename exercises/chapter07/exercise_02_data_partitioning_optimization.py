...
from pyspark.sql.functions import col, when, count, avg, sum as spark_sum, max as spark_max, \
    min as spark_min, stddev, spark_partition_id, size, explode, \
    rand, round as spark_round, desc, asc, row_number, \
    expr, broadcast, collect_list
...
def main():
    ...
    def analyze_partitions(df, name="DataFrame"):
        ...
        # 數據傾斜檢測
        if partition_stats['max_records_per_partition'] > partition_stats['avg_records_per_partition'] * 2:
            print("⚠️  檢測到數據傾斜!")
        else:
            print("✓ 數據分佈相對均勻")
    ...
    def create_large_dataset(num_records=100000):
        ...
        columns = ["id", "user_name", "group_id", "category", "age", "amount", "country"]
    ...
    def compare_partitioning_methods():
        ...
        for name, df in strategies.items():
            ...
            # 執行聚合操作
            result = df.groupBy("group_id", "category").agg(
                count("*").alias("count"),
        ...
        for result in results:
            print(f"{result['strategy']:<20}\t{result['num_partitions']}\t{result['execution_time']:.2f}")
    ...
    def test_join_partitioning():
        ...
        join_strategies = [
            ...
            ("兩表都按key分區", large_df.repartition("group_id"), small_df.repartition("group_id")),
        ...
        for name, left_df, right_df in join_strategies:
            ...
            # 執行聚合以觸發計算
            result = joined_df.groupBy("tier", "category").agg(
                count("*").alias("count"),
                avg("amount").alias("avg_amount"),
                spark_sum(col("amount") * col("discount_rate")).alias("discounted_total")
            ).collect()
        ...
        for result in join_results:
            print(f"{result['strategy']:<20}\t{result['execution_time']:.2f}\t\t{result['result_count']}")
    ...
    def handle_data_skew():
        ...
        # 對小表展開(與鹽值匹配)
        expanded_small = small_df.withColumn("salt", explode(expr("sequence(0, 4)"))) \
                                .withColumn("salted_key", expr("concat(group_id, '_', salt)"))
        
        start_time = time.time()
        
        # 使用加鹽後的key進行JOIN
        salted_join = salted_large.join(expanded_small, "salted_key", "inner") \
        ...
        # 識別熱點key
        hot_keys = group_distribution.filter(col("count") > 1000).select("group_id").rdd.flatMap(lambda x: x).collect()
        ...
        # 合併結果
        combined_result = normal_join.union(hot_join) \
                                   .groupBy("tier", "category") \
                                   .agg(count("*").alias("count"),
                                        avg("amount").alias("avg_amount")) \
                                   .collect()
    ...
    def partition_performance_analysis(df):
        ...
        partition_analysis = df.withColumn("partition_id", spark_partition_id()) \
                              ...
                              .withColumn("load_factor", 
                                        col("record_count") / 
                                        expr("(SELECT AVG(record_count) FROM (SELECT partition_id, COUNT(*) as record_count FROM table GROUP BY partition_id))")) \
                              .orderBy("partition_id")
