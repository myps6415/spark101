...
from pyspark.sql.functions import (array, asc, avg, broadcast, col,
                                   collect_list, count, date_format, dayofweek,
                                   dense_rank, desc, explode, expr, first, lag,
                                   last, lead)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import month, rand
from pyspark.sql.functions import round as spark_round
from pyspark.sql.functions import row_number, stddev, struct
from pyspark.sql.functions import sum as spark_sum
from pyspark.sql.functions import to_timestamp, when, window, year
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("SQL執行計劃優化練習")
        .master("local[*]")
    ...
    def test_join_strategies():
        ...
        join_strategies = [
            ...
            (
                "Shuffle Hash Join",
                base_query.replace(
                    "JOIN customers c", "JOIN /*+ SHUFFLEHASH(c) */ customers c"
                ),
            ),
            (
                "Sort Merge Join",
                base_query.replace(
                    "JOIN customers c", "JOIN /*+ MERGE(c) */ customers c"
                ),
            ),
        ]
        ...
        for result in join_results:
            print(
                f"{result['strategy']:<25}\t{result['execution_time']:.2f}\t\t{result['result_count']}"
            )
    ...
    def collect_table_statistics():
        ...
        for table in tables:
            ...
            try:
                ...
                stats_df.filter(
                    col("col_name").isin(
                        ["Statistics", "Table Size Bytes", "Num Files"]
                    )
                ).show(truncate=False)
            except Exception as e:
    ...
    def generate_tuning_recommendations():
        ...
        recommendations.append(
            f"推薦JOIN策略: {best_join['strategy']} (執行時間: {best_join['execution_time']:.2f}s)"
        )

        # 基於聚合測試結果
        best_agg = min(aggregation_results, key=lambda x: x["execution_time"])
        recommendations.append(
            f"推薦聚合策略: {best_agg['strategy']} (執行時間: {best_agg['execution_time']:.2f}s)"
        )

        # 基於子查詢測試結果
        best_subquery = min(subquery_results, key=lambda x: x["execution_time"])
        recommendations.append(
            f"推薦子查詢重寫: {best_subquery['method']} (執行時間: {best_subquery['execution_time']:.2f}s)"
        )

        # 基於Catalyst配置結果
        best_catalyst = min(catalyst_results, key=lambda x: x["execution_time"])
        recommendations.append(
            f"推薦優化器配置: {best_catalyst['config']} (執行時間: {best_catalyst['execution_time']:.2f}s)"
        )
