...
from pyspark.sql.functions import col, when, count, avg, sum as spark_sum, max as spark_max, \
    ...
    expr, current_timestamp, unix_timestamp, lit, \
...
def main():
    ...
    def get_spark_storage_info():
        ...
        print(f"Spark 內存使用率: {(used_memory / total_memory * 100) if total_memory > 0 else 0:.1f}%")
    ...
    def generate_complex_dataset(num_records=500000):
        ...
        columns = ["id", "user_name", "category", "score", "amount", "country", "timestamp", "is_active"]
        return spark.createDataFrame(data, columns)
    
    def create_complex_transformations(df):
        """創建複雜的數據轉換"""
        return df.withColumn("computed_score", 
                           col("score") * col("amount") / 100) \
    ...
    def cache_performance_test():
        ...
        for name, storage_level in storage_levels:
            ...
            # 計算改善率
            improvement = ((first_execution_time - second_execution_time) / first_execution_time * 100) if first_execution_time > 0 else 0
    ...
    def analyze_cache_results(results):
        ...
        for result in results:
            print(f"{result['storage_level']:<20}\t{result['first_time']:.2f}\t{result['second_time']:.2f}\t{result['improvement_percent']:.1f}\t\t{result['memory_increase']:.2f}")
        
        # 找出最佳存儲級別
        valid_results = [r for r in results if r['improvement_percent'] > 0]
        if valid_results:
            best_performance = max(valid_results, key=lambda x: x['improvement_percent'])
            ...
            print(f"最少內存使用: {best_memory['storage_level']} ({best_memory['memory_increase']:.2f} MB)")
    ...
    class SmartCacheManager:
        ...
        def cleanup_cache(self, max_age_minutes=30, min_usage=2):
            ...
            for name in to_remove:
                print(f"清理緩存: {name} (使用次數: {self.usage_counts[name]}, 年齡: {(current_time - self.last_access[name])/60:.1f}分鐘)")
    ...
    def dynamic_cache_management():
        ...
        for step_name, df in workflow_data:
            ...
            print(f"{step_name} 執行時間: {execution_time:.2f}s, 結果: {len(result)} 記錄")
    ...
    def memory_usage_analysis():
        ...
        # 創建不同類型的數據進行緩存
        datasets = {
            "數值密集": generate_complex_dataset(100000).select("id", "score", "amount"),
            "字符串密集": generate_complex_dataset(50000).select("id", "user_name", "country", "category"),
        ...
        for data_type, df in datasets.items():
            ...
            memory_per_record = (after_memory - before_memory) / count * 1024 * 1024  # bytes per record
            
            analysis_result = {
                "data_type": data_type,
                "record_count": count,
                "memory_increase_mb": after_memory - before_memory,
                "spark_memory_increase_mb": (after_spark_used - before_spark_used) / 1024 / 1024,
    ...
    def comprehensive_cache_benchmark():
        ...
        for scenario_name, df, usage in scenarios:
            ...
            if recommended_storage:
                ...
                improvement = ((no_cache_time - second_cache_time) / no_cache_time * 100) if no_cache_time > 0 else 0
    ...
    print(f"Spark內存變化: {(final_spark_used - initial_spark_used) / 1024 / 1024:.2f} MB")
    
    # 生成性能報告
    performance_summary = {
        "最佳緩存策略": best_storage_level,
        "最大性能改善": f"{best_improvement:.1f}%",
        "內存效率最高": min(memory_analysis, key=lambda x: x['bytes_per_record'])['data_type'],
