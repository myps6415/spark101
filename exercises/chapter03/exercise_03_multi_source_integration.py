...
from pyspark.sql.functions import (avg, coalesce, col, concat_ws, count,
                                   current_timestamp, isnan, isnull, lit,
                                   lower)
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("多數據源整合練習")
        .master("local[*]")
        .getOrCreate()
    )
    ...
    print("\n1.1 讀取 JSON 銷售數據:")
    try:
        sales_df = spark.read.option("multiline", "true").json(
    ...
    sales_cleaned = (
        sales_df.withColumn("customer_name_clean", trim(upper(col("customer_name"))))
        .withColumn("category_clean", trim(upper(col("category"))))
    ...
    print("清洗後的銷售數據:")
    sales_cleaned.select(
        "order_id", "customer_name_clean", "category_clean", "total_amount"
    ).show()
    ...
    employees_cleaned = (
        employees_df.withColumn("name_clean", trim(upper(col("name"))))
    ...
    print("清洗後的員工數據:")
    employees_cleaned.select(
        "name_clean", "department_clean", "city_clean", "salary"
    ).show()
    ...
    logs_parsed = (
        logs_df.withColumn(
            "timestamp",
            regexp_replace(
                col("value"), r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}).*", "$1"
            ),
        )
        .withColumn(
            "level",
            regexp_replace(col("value"), r".*(\d{2}:\d{2}:\d{2})\s+(\w+)\s+.*", "$2"),
        )
        .withColumn(
            "message",
            regexp_replace(col("value"), r".*\d{2}:\d{2}:\d{2}\s+\w+\s+(.*)", "$1"),
        )
        .filter(col("level").isin(["INFO", "WARN", "ERROR", "DEBUG"]))
    )
    ...
    sales_geo_analysis = (
        integrated_sales_emp.filter(col("employee_city").isNotNull())
    ...
    dept_consumption = (
        integrated_sales_emp.filter(col("employee_department").isNotNull())
    ...
    order_logs = logs_parsed.filter(
        ...
        "extracted_order_id", regexp_replace(col("message"), r".*Order\s+(\S+).*", "$1")
    )

    print("提取的訂單日誌:")
    order_logs.select("timestamp", "level", "extracted_order_id", "message").show(
        truncate=False
    )
    ...
    completeness_report = {
        ...
        "customers_with_employee_info": integrated_sales_emp.filter(
            col("employee_department").isNotNull()
        ).count(),
        "orders_with_logs": sales_with_logs.filter(
            col("has_log_record") == True
        ).count(),
    }
    ...
    total_sales_customers = (
        sales_cleaned.select("customer_name_clean").distinct().count()
    )
    matched_customers = (
        integrated_sales_emp.filter(col("employee_department").isNotNull())
    ...
    match_rate = (
        (matched_customers / total_sales_customers * 100)
        if total_sales_customers > 0
        else 0
    )

    print(
        f"客戶姓名匹配率: {match_rate:.1f}% ({matched_customers}/{total_sales_customers})"
    )
    ...
    dashboard_metrics = {
        ...
        "employee_customers": integrated_sales_emp.filter(
            col("employee_department").isNotNull()
        )
        .select("customer_id")
        .distinct()
        ...
        "cities_covered": integrated_sales_emp.filter(col("employee_city").isNotNull())
    ...
    try:
        ...
        customer_360_view.coalesce(1).write.mode("overwrite").parquet(
            f"{output_path}/customer_360_view"
        )

        sales_with_logs.coalesce(1).write.mode("overwrite").parquet(
            f"{output_path}/sales_with_logs"
        )

        master_customers.coalesce(1).write.mode("overwrite").parquet(
            f"{output_path}/master_customers"
        )
    ...
    lineage_info = {
        ...
        "output_datasets": ["customer_360_view", "sales_with_logs", "master_customers"],
        "quality_checks": [
    ...
    integration_summary = {
        "數據源數量": 3,
        "總記錄數": sales_cleaned.count()
        + employees_cleaned.count()
        + logs_parsed.count(),
