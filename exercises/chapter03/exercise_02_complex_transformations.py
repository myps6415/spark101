...
from pyspark.sql.functions import (array, avg, ceil, col, collect_list, concat,
                                   concat_ws, count, date_format, dayofmonth,
                                   dense_rank, explode, first, floor, hour,
                                   lag, last, lead, lower, map_keys,
                                   map_values)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import (minute, month, percentile_approx, rank,
                                   regexp_extract, regexp_replace)
from pyspark.sql.functions import round as spark_round
from pyspark.sql.functions import (row_number, size, sort_array, split, stddev,
                                   struct)
from pyspark.sql.functions import sum as spark_sum
from pyspark.sql.functions import trim, upper, when, year
from pyspark.sql.types import (ArrayType, DoubleType, IntegerType, MapType,
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("複雜數據轉換和聚合練習")
        .master("local[*]")
        .getOrCreate()
    )
    ...
    orders_with_categories = orders_df.withColumn(
        "category_array",
        split(
            regexp_replace(regexp_replace(col("categories"), "\\[", ""), "\\]", ""),
            ", ",
        ),
    )

    print("解析後的分類數組:")
    orders_with_categories.select("order_id", "categories", "category_array").show(
        truncate=False
    )
    ...
    category_stats = (
        orders_exploded.groupBy("category")
        .agg(
            count("*").alias("order_count"),
    ...
    ).withColumn("spending_dense_rank", dense_rank().over(window_rank))
    ...
    orders_time_analysis = (
        orders_df.withColumn("order_sequence", row_number().over(window_customer_time))
        .withColumn(
    ...
    # 手動創建透視表效果
    pivot_data = orders_df.groupBy("channel").agg(
        spark_sum(
            when(col("status") == "completed", col("total_amount")).otherwise(0)
        ).alias("completed_amount"),
        spark_sum(
            when(col("status") == "pending", col("total_amount")).otherwise(0)
        ).alias("pending_amount"),
        spark_sum(
            when(col("status") == "processing", col("total_amount")).otherwise(0)
        ).alias("processing_amount"),
        spark_sum(
            when(col("status") == "cancelled", col("total_amount")).otherwise(0)
        ).alias("cancelled_amount"),
    ...
    customer_analysis = (
        enriched_orders.groupBy("customer_id", "name", "tier", "city", "age")
        .agg(
    ...
    orders_time_breakdown = (
        orders_df.withColumn("order_year", year(col("order_date")))
    ...
    # 按日期聚合
    daily_stats = (
        orders_time_breakdown.groupBy("order_year", "order_month", "order_day")
        .agg(
    ...
    hourly_stats = (
        orders_time_breakdown.groupBy("order_hour")
        .agg(
    ...
    bucket_analysis = (
        amount_buckets.groupBy("amount_bucket")
        .agg(
            count("*").alias("order_count"),
    ...
    completeness_check = orders_df.agg(
        ...
        spark_sum(when(col("total_amount") <= 0, 1).otherwise(0)).alias(
            "invalid_amounts"
        ),
        spark_sum(
            when(
                col("status").isin(["completed", "pending", "processing", "cancelled"]),
                0,
            ).otherwise(1)
    ...
    customer_consistency = orders_df.join(
        customers_df.select("customer_id"), "customer_id", "left"
    ).agg(
        count("*").alias("total_orders"),
        spark_sum(when(customers_df.customer_id.isNull(), 1).otherwise(0)).alias(
    ...
    completion_rate = (
        orders_df.filter(col("status") == "completed").count() / total_orders * 100
    )
    ...
    print(
        f"最熱門分類: {top_category['category']} ({top_category['order_count']} 筆訂單)"
    )
