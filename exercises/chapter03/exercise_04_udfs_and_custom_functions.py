...
from pyspark.sql.functions import col, when, array, struct, map_from_arrays, lit, \
...
def main():
    ...
    # 客戶交易數據
    transaction_data = [
        ("CUST001", "Alice Johnson", "alice@email.com", "2024-01-15", 1250.00, "electronics,books", "premium", 85),
        ("CUST002", "Bob Chen", "bob@email.com", "2024-01-16", 890.50, "clothing,accessories", "gold", 72),
        ("CUST003", "Charlie Wu", "charlie.wu@email.com", "2024-01-17", 2100.00, "electronics", "premium", 91),
        ("CUST004", "Diana Lin", "diana_lin@email.com", "2024-01-18", 567.25, "books,toys", "silver", 68),
        ("CUST005", "Eve Brown", "eve.brown@email.com", "2024-01-19", 1456.75, "clothing", "gold", 78),
        ("CUST006", "Frank Davis", "frank@email.com", "2024-01-20", 789.00, "electronics,home", "silver", 82),
        ("CUST007", "Grace Wilson", "grace.wilson@email.com", "2024-01-21", 345.50, "books", "bronze", 58),
        ("CUST008", "Henry Johnson", "henry@email.com", "2024-01-22", 1890.25, "electronics,clothing", "premium", 88),
        ("CUST009", "Ivy Lee", "ivy.lee@email.com", "2024-01-23", 623.75, "toys,books", "silver", 75),
        ("CUST010", "Jack Smith", "jack.smith@email.com", "2024-01-24", 1234.00, "home,electronics", "gold", 80)
    ]
    
    transaction_columns = ["customer_id", "name", "email", "transaction_date", "amount", "categories", "tier", "satisfaction_score"]
    ...
    df_string_processed = df.withColumn(
        ...
        "email_valid", validate_email_udf(col("email"))
    )
    
    print("字符串處理結果:")
    df_string_processed.select("name", "name_standardized", "email", "email_valid").show()
    ...
    df_value_processed = df_string_processed.withColumn(
        "customer_value_score", calculate_value_udf(col("amount"), col("satisfaction_score"), col("tier"))
    ).withColumn(
        "suggested_tier", suggest_upgrade_udf(col("tier"), col("amount"), col("satisfaction_score"))
    )
    
    print("數值計算結果:")
    df_value_processed.select("customer_id", "amount", "satisfaction_score", "tier", "customer_value_score", "suggested_tier").show()
    ...
    print("分類分析結果:")
    df_category_analyzed.select("customer_id", "categories", "category_analysis").show(truncate=False)
    ...
    df_risk_assessed = df_category_analyzed.withColumn(
        "risk_assessment", 
        assess_risk_udf(col("tier"), col("amount"), col("satisfaction_score"), col("email_valid"), col("categories"))
    )
    
    print("風險評估結果:")
    df_risk_assessed.select("customer_id", "tier", "amount", "satisfaction_score", "risk_assessment").show(truncate=False)
    ...
    # 營銷活動推薦
    def recommend_marketing_campaign(tier, amount, satisfaction, categories_str, risk_level):
        recommendations = []
    ...
    df_final = df_pandas_processed.withColumn(
        ...
        recommend_campaign_udf(col("tier"), col("amount"), col("satisfaction_score"), col("categories"), col("risk_assessment.risk_level"))
    ).withColumn(
        "lifecycle_stage",
        lifecycle_stage_udf(col("tier"), col("amount"), col("satisfaction_score"), col("category_analysis.count"))
    )
    
    print("營銷推薦和生命週期分析:")
    df_final.select("customer_id", "tier", "lifecycle_stage", "marketing_recommendations").show(truncate=False)
    ...
    print(f"- 有效郵箱比例: {df_string_processed.filter(col('email_valid') == True).count() / df.count() * 100:.1f}%")
    print(f"- 高風險客戶: {df_risk_assessed.filter(col('risk_assessment.risk_level') == 'HIGH').count()}")
    print(f"- 建議升級客戶: {df_value_processed.filter(col('suggested_tier') != col('tier')).count()}")
