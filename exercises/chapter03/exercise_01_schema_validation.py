...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("Schema和數據驗證練習")
        .master("local[*]")
        .getOrCreate()
    )
    ...
    products_with_date = products_df.withColumn(
        "launch_date_parsed", to_date(col("launch_date"), "yyyy-MM-dd")
    )
    ...
    # 6. 清洗數據
    cleaned_df = (
        products_with_date.fillna({"product_name": "Unknown Product"})
        ...
        .withColumn(
            "product_name",
            when(col("product_name") == "", "Unknown Product").otherwise(
                col("product_name")
            ),
        )
        .drop("launch_date")
    ...
    valid_names = cleaned_df.filter(
        col("product_name").isNotNull() & (col("product_name") != "Unknown Product")
    ).count()
    ...
    print(
        f"有效產品名稱: {valid_names}/{total_records} ({valid_names/total_records*100:.1f}%)"
    )
    print(
        f"有效價格: {valid_prices}/{total_records} ({valid_prices/total_records*100:.1f}%)"
    )
    print(
        f"有效評分: {valid_ratings}/{total_records} ({valid_ratings/total_records*100:.1f}%)"
    )
    print(
        f"有效日期: {valid_dates}/{total_records} ({valid_dates/total_records*100:.1f}%)"
    )
    ...
    print("庫存為0但標記為可用的產品:")
    availability_issues.select(
        "product_id", "product_name", "stock_quantity", "is_available"
    ).show()
