...
from pyspark.sql.functions import (add_months, avg, col, collect_list,
                                   concat_ws, count, cume_dist, date_format,
                                   datediff, dayofmonth, dense_rank, first,
                                   lag, last, lead)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import (month, nth_value, ntile, percent_rank, rank,
                                   row_number, stddev)
from pyspark.sql.functions import sum as spark_sum
from pyspark.sql.functions import var_pop, when, year
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("窗口函數和分析函數練習")
        .master("local[*]")
        .getOrCreate()
    )
    ...
    sales_with_global_rank = (
        sales_df.withColumn("global_rank", row_number().over(window_global))
        .withColumn("global_dense_rank", dense_rank().over(window_global))
        .withColumn("global_percent_rank", percent_rank().over(window_global))
    )
    ...
    # 按地區和月份分組排名
    window_region_month = Window.partitionBy("region", "month").orderBy(
        col("sales_amount").desc()
    )

    sales_with_region_rank = sales_df.withColumn(
        "region_month_rank", row_number().over(window_region_month)
    ).withColumn("region_month_dense_rank", dense_rank().over(window_region_month))
    ...
    sales_with_cumulative = (
        sales_df.withColumn(
            "cumulative_sales", spark_sum("sales_amount").over(window_emp_time)
        )
        .withColumn("cumulative_deals", spark_sum("deals_closed").over(window_emp_time))
        .withColumn("running_avg_sales", avg("sales_amount").over(window_emp_time))
        .withColumn(
    ...
    sales_with_sliding = (
        ...
        .withColumn("sliding_sales_stddev", stddev("sales_amount").over(window_sliding))
    )
    ...
    sales_with_comparison = (
        sales_df.withColumn(
            "prev_month_sales", lag("sales_amount", 1).over(window_lag_lead)
        )
        .withColumn("next_month_sales", lead("sales_amount", 1).over(window_lag_lead))
        .withColumn(
            "mom_growth",
            (
    ...
    sales_with_first_last = (
        sales_df.withColumn(
            "first_month_sales", first("sales_amount").over(window_first_last)
        )
        .withColumn("last_month_sales", last("sales_amount").over(window_first_last))
        .withColumn(
            "quarter_improvement", col("last_month_sales") - col("first_month_sales")
        )
        .withColumn(
            "quarter_improvement_pct",
            (col("last_month_sales") - col("first_month_sales"))
            / col("first_month_sales")
            * 100,
        )
    )
    ...
    multi_rank_analysis = (
        sales_df.withColumn("region_sales_rank", row_number().over(window_region))
        .withColumn("category_sales_rank", row_number().over(window_category))
        .withColumn(
            "avg_deal_rank_in_region",
            row_number().over(
                Window.partitionBy("region").orderBy(col("avg_deal_size").desc())
            ),
        )
    )
    ...
    relative_performance = (
        ...
        .withColumn(
            "sales_vs_region_avg",
            (col("sales_amount") - col("region_avg_sales"))
            / col("region_avg_sales")
            * 100,
        )
        .withColumn(
    ...
    trend_analysis = (
        ...
        .withColumn("two_month_ago_sales", lag("sales_amount", 2).over(window_trend))
        .withColumn("one_month_ago_sales", lag("sales_amount", 1).over(window_trend))
        .withColumn(
            "trend_direction",
            when(
            ...
            .when(col("sales_amount") > col("one_month_ago_sales"), "Up")
            .when(col("sales_amount") < col("one_month_ago_sales"), "Down")
            .otherwise("Stable"),
        )
    )
    ...
    print("各地區前2名:")
    top_performers.select("region", "emp_name", "sales_amount", "region_rank").orderBy(
        "region", "region_rank"
    ).show()
    ...
    performance_distribution = (
        ...
        .withColumn("deals_quartile", ntile(4).over(Window.orderBy("deals_closed")))
        .withColumn("combined_score", col("sales_quartile") + col("deals_quartile"))
        .withColumn(
            "overall_performance",
    ...
    print("績效分布:")
    performance_distribution.groupBy("overall_performance").count().orderBy(
        "count"
    ).show()
    ...
    comprehensive_analysis = (
        sales_df.withColumn(
            # 全局排名
            "global_sales_rank",
            row_number().over(Window.orderBy(col("sales_amount").desc())),
        )
        .withColumn(
            # 地區內排名
            "regional_rank",
            row_number().over(
                Window.partitionBy("region").orderBy(col("sales_amount").desc())
            ),
        )
        .withColumn(
            # 月度增長
            "prev_month_sales",
            lag("sales_amount", 1).over(Window.partitionBy("emp_id").orderBy("month")),
        )
        .withColumn(
            "growth_rate",
            (
        ...
        .withColumn(
            # 績效等級
            "performance_tier",
            ntile(3).over(Window.orderBy(col("sales_amount").desc())),
        )
    )
