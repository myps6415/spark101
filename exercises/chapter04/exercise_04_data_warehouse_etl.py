...
from pyspark.sql.functions import (avg, broadcast, coalesce, col, collect_list,
                                   concat, concat_ws, count, current_timestamp,
                                   date_add, date_format, date_sub, datediff,
                                   dayofmonth, dense_rank, first, isnull, last,
                                   lit)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import md5
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import (monotonically_increasing_id, month, quarter,
                                   rank, regexp_replace, row_number, sha1)
from pyspark.sql.functions import sum as spark_sum
from pyspark.sql.functions import trim, upper, when, year
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("數據倉庫操作和ETL練習")
        .master("local[*]")
    ...
    customer_quality = customers_df.agg(
        ...
        spark_sum(when(col("email").contains("@"), 0).otherwise(1)).alias(
            "invalid_emails"
        ),
        spark_sum(when(col("zip_code").rlike("^[0-9]{5}$"), 0).otherwise(1)).alias(
            "invalid_zip_codes"
        ),
    )
    ...
    # 清洗客戶數據
    customers_cleaned = (
        customers_df.withColumn("name_clean", trim(upper(col("name"))))
        .withColumn("email_clean", trim(lower(col("email"))))
        .withColumn("city_clean", trim(upper(col("city"))))
        .withColumn("state_clean", upper(col("state")))
        .filter(col("email").contains("@"))  # 過濾無效郵箱
    )

    # 清洗產品數據
    products_cleaned = (
        products_df.withColumn("category_clean", trim(upper(col("category"))))
        .withColumn("subcategory_clean", trim(upper(col("subcategory"))))
        .filter(col("price") > 0)  # 過濾無效價格
    )

    # 清洗交易數據
    transactions_cleaned = transactions_df.filter(
        (col("quantity") > 0) & (col("total_amount") > 0)
    ).withColumn("transaction_date_clean", col("transaction_date").cast(DateType()))
    ...
    dim_customer = customers_cleaned.select(
        ...
        md5(
            concat_ws("|", col("name_clean"), col("email_clean"), col("city_clean"))
        ).alias("record_hash"),
    )
    ...
    fact_sales = (
        transactions_cleaned.join(
            dim_customer.select("customer_id", "customer_key"), "customer_id", "inner"
        )
        .join(dim_product.select("product_id", "product_key"), "product_id", "inner")
        .join(
            dim_date.select("full_date", "date_key"),
            transactions_cleaned.transaction_date_clean == dim_date.full_date,
            "inner",
        )
        .select(
    ...
    # 清洗更新數據
    customers_updated_cleaned = (
        customers_updated_df.withColumn("name_clean", trim(upper(col("name"))))
        .withColumn("email_clean", trim(lower(col("email"))))
        .withColumn("city_clean", trim(upper(col("city"))))
        .withColumn("state_clean", upper(col("state")))
        .withColumn(
    ...
    changed_customers = customers_updated_cleaned.join(
        ...
        "customer_id",
        "inner",
    ).filter(customers_updated_cleaned.record_hash != existing_customers.record_hash)
    ...
    if changed_customers.count() > 0:
        ...
        dim_customer_expired = (
            ...
            .withColumn(
                "expiry_date",
                when(
                    changed_customers.customer_id.isNotNull(),
                    current_timestamp().cast(DateType()),
                ).otherwise(col("expiry_date")),
            )
            .withColumn(
                "is_current",
                when(changed_customers.customer_id.isNotNull(), False).otherwise(
                    col("is_current")
                ),
            )
        )
    ...
    sales_monthly_summary = (
        ...
        .join(dim_product.select("product_key", "category"), "product_key", "inner")
        .join(dim_customer.select("customer_key", "state"), "customer_key", "inner")
        .groupBy("year", "month", "quarter", "category", "state")
        .agg(
    ...
    customer_cube = (
        fact_sales.join(
            dim_customer.select(
                "customer_key", "customer_name", "state", "customer_status"
            ),
            "customer_key",
            "inner",
        )
        .join(dim_product.select("product_key", "category"), "product_key", "inner")
        .groupBy("customer_name", "state", "category", "customer_status")
        .agg(
    ...
    # 增量ETL處理
    incremental_fact = (
        new_transactions_df.join(
            dim_customer.filter(col("is_current") == True).select(
        ...
        .join(
            dim_date.select("full_date", "date_key"),
            new_transactions_df.transaction_date == dim_date.full_date,
            "inner",
        )
        .select(
            monotonically_increasing_id().alias("sales_fact_key"),
    ...
    quality_checks = {
        ...
        .filter(col("full_date") > current_timestamp().cast(DateType()))
        .count(),
    }
    ...
    print("分區策略設計:")
    partition_summary = (
        partitioned_fact_sales.groupBy("partition_key")
        .agg(
    ...
    table_stats = {
        ...
        "dim_product": {
            ...
            "active_products": dim_product.filter(
                col("product_status") == "Active"
            ).count(),
