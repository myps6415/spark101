...
from pyspark.sql.functions import (approx_count_distinct, array, avg,
                                   broadcast, case, ceil, coalesce, col,
                                   collect_list, collect_set, concat,
                                   concat_ws, corr, count, date_format,
                                   dayofweek, dense_rank, explode, floor,
                                   isnan, isnull, kurtosis, lag, lead)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import (month, ntile, percentile_approx, quarter,
                                   rank, regexp_replace)
from pyspark.sql.functions import round as spark_round
from pyspark.sql.functions import row_number, skewness, split, stddev, struct
from pyspark.sql.functions import sum as spark_sum
from pyspark.sql.functions import variance, when, year
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("複雜數據分析和報表練習")
        .master("local[*]")
        .getOrCreate()
    )
    ...
    monthly_trends = (
        orders_df.withColumn("order_month", date_format(col("order_date"), "yyyy-MM"))
        .groupBy("order_month")
        .agg(
    ...
    monthly_growth = (
        ...
        .withColumn("prev_month_orders", lag("monthly_orders", 1).over(window_monthly))
        .withColumn(
            "order_growth_rate",
            (
                (col("monthly_orders") - col("prev_month_orders"))
                / col("prev_month_orders")
                * 100
            ),
        )
    )
    ...
    seasonal_analysis = (
        orders_df.withColumn("quarter", quarter(col("order_date")))
    ...
    # 與客戶主數據關聯
    customer_enriched = customer_value_analysis.join(
    ...
    rfm_analysis = (
        ...
        .withColumn(
            "recency_days", datediff(lit(analysis_date), col("last_order_date"))
        )
        .withColumn(
            "recency_score",
        ...
        .withColumn(
            "customer_type",
            when(
                col("rfm_score").isin("555", "554", "544", "545", "454", "455", "445"),
                "Champions",
            )
            .when(
                col("rfm_score").isin(
            ...
            .when(
                col("rfm_score").isin("512", "511", "422", "421", "412", "411", "311"),
                "Potential Loyalists",
            )
            .when(
                col("rfm_score").isin(
            ...
            .when(
                col("rfm_score").isin("155", "154", "144", "214", "215", "115", "114"),
                "At Risk",
            )
            .when(
                col("rfm_score").isin("155", "154", "144", "214", "215", "115"),
                "Cannot Lose Them",
            )
            .otherwise("Others"),
        )
    ...
    category_performance = (
        ...
        .withColumn("items_per_order", col("total_items") / col("order_count"))
    )
    ...
    window_customer = Window.partitionBy("customer_id").orderBy(
        col("orders_in_channel").desc()
    )

    primary_channel = (
        customer_channel_preference.withColumn(
            "channel_rank", row_number().over(window_customer)
        )
        .filter(col("channel_rank") == 1)
        .select(
            "customer_id",
            col("channel").alias("primary_channel"),
            "orders_in_channel",
            "amount_in_channel",
        )
    )
    ...
    # 與客戶信息合併
    customer_channel_analysis = primary_channel.join(
        customers_df, "customer_id", "inner"
    )
    ...
    # 基於歷史數據預測客戶價值
    clv_analysis = (
        customer_enriched.withColumn(
            "avg_monthly_spend", col("total_spent") / 3  # 假設3個月數據
        )
        .withColumn("predicted_annual_value", col("avg_monthly_spend") * 12)
        .withColumn(
    ...
    channel_summary = (
        orders_df.filter(col("status") == "completed")
        .groupBy("channel")
        .agg(count("*").alias("orders"), spark_sum("amount").alias("revenue"))
        .collect()
    )
