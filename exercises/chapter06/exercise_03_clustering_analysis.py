...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("聚類分析練習")
        .master("local[*]")
    ...
    # 按標籤統計
    label_stats = (
        cluster_labels.groupBy("cluster_label")
        .agg(
    ...
    # 計算客戶生命週期價值
    clv_analysis = (
        best_clustered_df.withColumn(
            "estimated_clv",
            col("monthly_purchases")
            * col("avg_order_value")
            * 12
            * 2,  # 假設2年生命週期
        )
    ...
    strategy_summary = marketing_strategies.groupBy(
        "cluster_label", "marketing_strategy"
    ).count()
    print("營銷策略分配:")
    ...
    print(
        f"穩定性評級: {'高' if std_stability < 0.05 else '中' if std_stability < 0.1 else '低'}"
    )
    ...
    recommendation_rules = best_clustered_df.withColumn(
        ...
        when(
            col("cluster_label") == "Premium Shoppers",
            array(lit("高端電子產品"), lit("奢侈品"), lit("專業設備")),
        )
        .when(
            col("cluster_label") == "High Value Customers",
            array(lit("智能家居"), lit("高品質服裝"), lit("專業書籍")),
        )
        .when(
            col("cluster_label") == "Regular Customers",
            array(lit("日用品"), lit("休閒服裝"), lit("娛樂產品")),
        )
        .when(
            col("cluster_label") == "Budget Conscious",
    ...
    print("個性化推薦示例:")
    recommendation_rules.select(
        "customer_id", "cluster_label", "product_recommendation", "promotion_type"
