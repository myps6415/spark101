...
from pyspark.sql.functions import col, when, count, avg, sum as spark_sum, max as spark_max, \
    min as spark_min, stddev, sqrt, pow as spark_pow, abs as spark_abs, \
    rand, round as spark_round, desc, asc, row_number, \
    percentile_approx, expr, isnan, isnull
...
def main():
    ...
    def generate_normal_transactions():
        ...
        for i in range(1, 1001):  # 1000筆正常交易
            ...
            transactions.append((
                i, amount, transaction_hour, merchant_category,
                location_risk_score, distance_from_home, frequency_last_hour,
                avg_amount_last_30_days, is_online, device_risk_score, 0  # 0 = 正常
            ))
    ...
    def generate_anomalous_transactions():
        ...
        for i in range(1001, 1201):  # 200筆異常交易
            ...
            anomalies.append((
                i, amount, transaction_hour, merchant_category,
                location_risk_score, distance_from_home, frequency_last_hour,
                avg_amount_last_30_days, is_online, device_risk_score, 1  # 1 = 異常
            ))
    ...
    transaction_columns = [
        ...
        "location_risk_score", "distance_from_home", "frequency_last_hour",
        "avg_amount_last_30_days", "is_online", "device_risk_score", "is_fraud"
    ]
    ...
    z_score_df = transactions_df.withColumn(
        "z_score",
        abs((col("amount") - mean_amount) / std_amount)
    ).withColumn(
        "is_outlier_zscore",
        when(col("z_score") > 3, 1).otherwise(0)  # 3-sigma 規則
    )
    ...
    z_score_accuracy = z_score_df.select(
        avg(when(col("is_fraud") == col("is_outlier_zscore"), 1).otherwise(0)).alias("accuracy")
    ).collect()[0]["accuracy"]
    ...
    iqr_df = transactions_df.withColumn(
        "is_outlier_iqr",
        when((col("amount") < lower_bound) | (col("amount") > upper_bound), 1).otherwise(0)
    )
    ...
    iqr_accuracy = iqr_df.select(
        avg(when(col("is_fraud") == col("is_outlier_iqr"), 1).otherwise(0)).alias("accuracy")
    ).collect()[0]["accuracy"]
    ...
    feature_columns = [
        ...
        "location_risk_score", "distance_from_home", "frequency_last_hour",
        "avg_amount_last_30_days", "device_risk_score"
    ]
    ...
    cluster_accuracy = cluster_anomaly_df.select(
        avg(when(col("is_fraud") == col("is_outlier_cluster"), 1).otherwise(0)).alias("accuracy")
    ).collect()[0]["accuracy"]
    ...
    # 合併所有方法的結果
    ensemble_df = cluster_anomaly_df.join(
        z_score_df.select("transaction_id", "is_outlier_zscore"), "transaction_id"
    ).join(
        iqr_df.select("transaction_id", "is_outlier_iqr"), "transaction_id"
    ).join(
        lr_predictions.select("transaction_id", "prediction").withColumnRenamed("prediction", "lr_prediction"),
        "transaction_id"
    )
    ...
    ensemble_score_df = ensemble_df.withColumn(
        "ensemble_score",
        col("is_outlier_cluster") + col("is_outlier_zscore") + 
        col("is_outlier_iqr") + col("lr_prediction")
    ).withColumn(
        "is_outlier_ensemble",
    ...
    ensemble_accuracy = ensemble_score_df.select(
        avg(when(col("is_fraud") == col("is_outlier_ensemble"), 1).otherwise(0)).alias("accuracy")
    ).collect()[0]["accuracy"]
    ...
    realtime_df = transactions_df.withColumn(
        ...
        when((col("transaction_hour") < 6) | (col("transaction_hour") > 23), 0.2).otherwise(0) +
    ...
    monitoring_rules = realtime_df.withColumn(
        "rule_triggers",
        when(col("amount") > 2000, array(lit("large_amount")))
        .otherwise(array()) +
        when((col("transaction_hour") < 6) | (col("transaction_hour") > 23), array(lit("unusual_time")))
        .otherwise(array()) +
        when(col("location_risk_score") > 0.7, array(lit("high_risk_location")))
        .otherwise(array()) +
        when(col("frequency_last_hour") > 8, array(lit("high_frequency")))
        .otherwise(array()) +
        when(col("distance_from_home") > 500, array(lit("unusual_location")))
        .otherwise(array())
    ).withColumn(
        "alert_level",
    ...
    print("各方法準確率比較:")
    for method, acc in sorted(method_performance.items(), key=lambda x: x[1], reverse=True):
    ...
    tp = best_method_df.filter((col("is_fraud") == 1) & (col("is_outlier_ensemble") == 1)).count()
    fp = best_method_df.filter((col("is_fraud") == 0) & (col("is_outlier_ensemble") == 1)).count()
    tn = best_method_df.filter((col("is_fraud") == 0) & (col("is_outlier_ensemble") == 0)).count()
    fn = best_method_df.filter((col("is_fraud") == 1) & (col("is_outlier_ensemble") == 0)).count()
    ...
    fraud_analysis = transactions_df.filter(col("is_fraud") == 1).withColumn(
        ...
        .when((col("transaction_hour") < 6) | (col("transaction_hour") > 23), "Unusual Time")
    ...
    new_trans_df = spark.createDataFrame(new_transactions, [
        "transaction_id", "amount", "transaction_hour", "merchant_category",
        "location_risk_score", "distance_from_home", "frequency_last_hour",
        "avg_amount_last_30_days", "is_online", "device_risk_score"
    ])
    ...
    final_predictions = new_distances.join(
        ...
        "is_anomaly",
        when(
            (col("distance_to_center") > distance_threshold) |
            (col("prediction") == 1),
            True
        ).otherwise(False)
    )
    ...
    final_predictions.select(
        ...
        "distance_to_center", "prediction", "is_anomaly"
    ).show()
