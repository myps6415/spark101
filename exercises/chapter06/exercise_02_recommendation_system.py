...
from pyspark.sql.functions import (array_contains, asc, avg, col, collect_list,
                                   count, dense_rank, desc, explode)
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("推薦系統練習")
        .master("local[*]")
    ...
    print(
        f"最佳參數 - Rank: {best_model.rank}, RegParam: {best_model._java_obj.regParam()}"
    )
    ...
    user_recs_with_products = user_recs_exploded.join(
        products_df.select("product_id", "name", "category", "price"), "product_id"
    ).join(
    ...
    print("為用戶 1-3 的推薦:")
    user_recs_with_products.filter(col("user_id").isin([1, 2, 3])).orderBy(
        "user_id", desc("predicted_rating")
    ...
    item_recs_exploded = item_recommendations.select(
        col("product_id"), explode(col("recommendations")).alias("recommendation")
    ).select(
        col("product_id"),
    ...
    item_recs_with_info = item_recs_exploded.join(
        products_df.select("product_id", "name").withColumnRenamed(
            "name", "product_name"
        ),
        "product_id",
    ).join(users_df.select("user_id", "name", "occupation"), "user_id")

    print("為商品推薦用戶 (前3個商品):")
    item_recs_with_info.filter(col("product_id").isin([101, 102, 103])).orderBy(
        "product_id", desc("predicted_rating")
    ).show(truncate=False)
    ...
    def get_category_recommendations(user_id, num_recs=3):
        # 獲取用戶已評分的商品類別
        user_categories = (
            ratings_df.filter(col("user_id") == user_id)
        ...
        if user_categories:
            ...
            recommendations = (
                ...
                .join(product_stats.select("product_id", "avg_rating"), "product_id")
    ...
    def hybrid_recommendations(user_id, num_recs=5):
        # 協同過濾推薦 (權重 0.7)
        cf_recs = (
            user_recs_with_products.filter(col("user_id") == user_id)
        ...
        # 基於內容的推薦 (權重 0.3)
        content_recs = (
            get_category_recommendations(user_id, 10)
        ...
        # 合併推薦
        hybrid_recs = (
            cf_recs.join(content_recs, "product_id", "outer")
            .fillna(0)
            .withColumn("hybrid_score", col("cf_score") + col("content_score"))
            .join(
    ...
    # 計算用戶推薦的類別多樣性
    diversity_analysis = (
        user_recs_with_products.groupBy("user_id")
        .agg(
        ...
        .withColumn(
            "diversity_score", col("unique_categories") / col("total_recommendations")
        )
    )

    print("推薦多樣性分析:")
    diversity_analysis.select(
        "user_id", "total_recommendations", "unique_categories", "diversity_score"
    ).show()
    ...
    recommended_products = (
        user_recs_with_products.select("product_id").distinct().count()
    )
    ...
    # 長尾商品推薦分析
    popular_products = product_stats.filter(col("rating_count") >= 3).select(
        "product_id"
    )
    long_tail_products = product_stats.filter(col("rating_count") < 3).select(
        "product_id"
    )

    popular_in_recs = user_recs_with_products.join(
        popular_products, "product_id", "inner"
    ).count()
    long_tail_in_recs = user_recs_with_products.join(
        long_tail_products, "product_id", "inner"
    ).count()

    print(f"熱門商品推薦數: {popular_in_recs}")
    print(f"長尾商品推薦數: {long_tail_in_recs}")
    print(
        f"長尾商品推薦比例: {long_tail_in_recs/(popular_in_recs + long_tail_in_recs)*100:.2f}%"
    )
    ...
    popularity_recommendations = (
        product_stats.filter(col("rating_count") >= 2)
    ...
    print("新用戶熱門商品推薦:")
    popularity_recommendations.select(
        "product_id", "name", "category", "avg_rating", "rating_count"
    ).show()
    ...
    def recommend_new_product(product_id):
        ...
        # 找到該類別中評分較高的用戶
        similar_category_users = (
            ratings_df.join(
                products_df.filter(col("category") == product_info["category"]).select(
                    "product_id"
                ),
                "product_id",
            )
            .groupBy("user_id")
            .agg(avg("rating").alias("avg_category_rating"))
