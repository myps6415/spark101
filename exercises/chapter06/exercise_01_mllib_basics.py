...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("MLlib基礎練習").master("local[*]").getOrCreate()
    )
    ...
    # 檢查缺失值
    missing_count = employees_df.select(
        [
    ...
    employees_with_target = employees_df.withColumn(
        "high_salary", when(col("salary") > avg_salary, 1).otherwise(0)
    )
    ...
    print("\n預測結果:")
    predictions.select(
        "name", "salary", "high_salary", "probability", "prediction"
    ...
    evaluator = BinaryClassificationEvaluator(
        labelCol="high_salary", metricName="areaUnderROC"
    )
    ...
    rf = RandomForestClassifier(
        featuresCol="scaled_features", labelCol="high_salary", numTrees=10
    )
    ...
    regression_pipeline = Pipeline(
        stages=[dept_indexer, city_indexer, assembler, scaler, lr_reg]
    )
