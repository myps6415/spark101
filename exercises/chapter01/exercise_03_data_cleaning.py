...
from pyspark.sql.functions import (avg, coalesce, col, isnan, isnull, lit,
                                   lower, regexp_extract, regexp_replace,
                                   split, stddev, trim, upper, when)
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("數據清洗和轉換練習")
        .master("local[*]")
        .getOrCreate()
    )
    ...
    for column in dirty_df.columns:
        null_count = dirty_df.filter(
            col(column).isNull() | (col(column) == "") | (col(column) == " ")
        ).count()
    ...
    try:
        invalid_scores = dirty_df.filter(
            (col("score").cast("double") < 0) | (col("score").cast("double") > 100)
        ).count()
    ...
    cleaned_step1 = dirty_df.withColumn(
        "name_cleaned", trim(regexp_replace(col("name"), "\\s+", " "))  # 移除多餘空格
    )
    ...
    # 計算有效年齡的平均值用於填補
    valid_ages = dirty_df.filter(col("age").rlike("^[0-9]+$")).select(
        col("age").cast("int")
    )
    ...
    cleaned_step2 = cleaned_step1.withColumn(
        "age_cleaned",
        when(col("age").rlike("^[0-9]+$"), col("age").cast("int")).otherwise(
            int(avg_age)
        ),  # 用平均值填補無效年齡
    )
    ...
    cleaned_step3 = cleaned_step2.withColumn(
        "department_cleaned", lower(trim(col("department")))
    )
    ...
    for old_name, new_name in department_mapping.items():
        cleaned_step3 = cleaned_step3.withColumn(
            "department_cleaned",
            when(col("department_cleaned") == old_name, new_name).otherwise(
    ...
    cleaned_step3 = cleaned_step3.withColumn(
        "department_cleaned",
        when(
            (col("department_cleaned").isNull()) | (col("department_cleaned") == ""),
            "Unknown Department",
    ...
    # 計算有效成績的統計值
    valid_scores = dirty_df.filter(col("score").rlike("^[0-9]+\\.?[0-9]*$")).select(
        col("score").cast("double").alias("score_num")
    )

    score_stats = valid_scores.agg(
        avg("score_num").alias("avg_score"), stddev("score_num").alias("std_score")
    ).collect()[0]
    ...
    cleaned_step4 = cleaned_step3.withColumn(
        "score_cleaned",
        when(
            col("score").rlike("^[0-9]+\\.?[0-9]*$"), col("score").cast("double")
        ).otherwise(
            avg_score
        ),  # 用平均值填補無效成績
    )
    ...
    cleaned_step4 = cleaned_step4.withColumn(
        "score_cleaned",
        when(
            (col("score_cleaned") < 0) | (col("score_cleaned") > 100), avg_score
    ...
    cleaned_final = cleaned_step4.withColumn(
        "email_cleaned",
        when(
            col("email").rlike(email_pattern), lower(trim(col("email")))  # 標準化為小寫
        ).otherwise(
            "invalid@email.com"
        ),  # 無效郵箱用預設值
    )
    ...
    quality_check = {
        ...
        "未知科系": final_cleaned.filter(
            col("department") == "Unknown Department"
        ).count(),
        "無效成績": final_cleaned.filter(
    ...
    # 按科系統計清理後的數據
    print("\n科系分布統計:")
    final_complete.groupBy("department").count().orderBy(
