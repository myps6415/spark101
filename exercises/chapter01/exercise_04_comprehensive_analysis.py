...
from pyspark.sql.functions import (asc, avg, col, count, dense_rank, desc, lag,
                                   lead)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import percentile_approx, rank
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("綜合分析練習").master("local[*]").getOrCreate()
    )
    ...
    print("\n2.2 年齡分布分析:")
    age_distribution = (
        students_df.groupBy("age")
        .agg(count("*").alias("student_count"), avg("score").alias("avg_score"))
        .orderBy("age")
    )
    ...
    top_students_by_dept = (
        students_df.withColumn("rank_in_dept", row_number().over(window_dept))
    ...
    print("\n城市-科系交叉分析:")
    city_dept_analysis = (
        students_df.groupBy("city", "department")
        .agg(count("*").alias("student_count"), avg("score").alias("avg_score"))
    ...
    semester_analysis = (
        students_df.groupBy("semester", "year")
        .agg(
            count("*").alias("student_count"),
            avg("score").alias("avg_score"),
            avg("gpa").alias("avg_gpa"),
    ...
    grade_distribution = (
        students_with_grade.groupBy("grade")
        .agg(
            count("*").alias("student_count"),
    ...
    print("\n各科系成績等級分布:")
    grade_by_dept = (
        students_with_grade.groupBy("department", "grade")
        .count()
    ...
    ).withColumn("dense_rank", dense_rank().over(window_overall))

    print("前10名學生:")
    students_with_rank.filter(col("overall_rank") <= 10).select(
        "overall_rank", "name", "department", "score", "gpa"
    ).show()
    ...
    print(
        f"最優秀科系: {best_dept['department']} (平均成績: {best_dept['avg_score']:.2f})"
    )
    ...
    # 成績分布特徵
    a_grade_pct = grade_distribution.filter(col("grade") == "A").collect()[0][
        "percentage"
    ]
    ...
    trend_analysis = students_df.withColumn(
        "prev_score", lag("score", 1).over(window_trend)
    ).withColumn("score_diff", col("score") - col("prev_score"))

    # 顯示有前一個比較基準的記錄
    print("科系內成績變化趨勢 (按姓名排序):")
    trend_analysis.filter(col("prev_score").isNotNull()).select(
        "name", "department", "score", "prev_score", "score_diff"
    ).show()
    ...
    # 創建最終分析結果
    final_analysis = (
        students_with_grade.join(
            students_with_rank.select("name", "overall_rank"), "name"
        )
        .select(
            "name",
            "age",
