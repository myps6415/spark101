...
def main():
    ...
    # 6. 按範圍分組
    range_groups = even_squares.map(lambda x: (
        "Small" if x < 1000 else "Medium" if x < 5000 else "Large", 1
    ...
    numbers_rdd.filter(lambda x: x % 2 == 0).map(lambda x: x ** 2).reduce(lambda x, y: x + y)
    end_time = time.time()
