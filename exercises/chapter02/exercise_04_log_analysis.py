...
def main():
    ...
    for i, log in enumerate(parsed_logs.take(3), 1):
        print(f"{i}. IP: {log['ip']}, Method: {log['method']}, URL: {log['url']}, Status: {log['status_code']}")
    ...
    # 3.1 狀態碼分布
    print("\n3.1 HTTP 狀態碼分布:")
    status_counts = parsed_logs.map(lambda x: (x['status_code'], 1)) \
    ...
    # 3.2 請求方法統計
    print("\n3.2 HTTP 請求方法統計:")
    method_counts = parsed_logs.map(lambda x: (x['method'], 1)) \
                              .reduceByKey(lambda a, b: a + b) \
    ...
    # 4.1 訪問頻率最高的 IP
    print("\n4.1 訪問頻率最高的 IP 地址:")
    ip_counts = parsed_logs.map(lambda x: (x['ip'], 1)) \
                          .reduceByKey(lambda a, b: a + b) \
                          .sortBy(lambda x: x[1], ascending=False)
    ...
    # 4.2 每個 IP 的流量統計
    print("\n4.2 每個 IP 的流量統計:")
    ip_traffic = parsed_logs.map(lambda x: (x['ip'], x['response_size'])) \
                           .reduceByKey(lambda a, b: a + b) \
                           .sortBy(lambda x: x[1], ascending=False)
    ...
    # 5.1 最熱門的 URL
    print("\n5.1 最熱門的 URL:")
    url_counts = parsed_logs.map(lambda x: (x['url'], 1)) \
                           .reduceByKey(lambda a, b: a + b) \
                           .sortBy(lambda x: x[1], ascending=False)
    ...
    api_counts = api_logs.map(lambda x: (x['url'], 1)) \
                        .reduceByKey(lambda a, b: a + b) \
                        .sortBy(lambda x: x[1], ascending=False)
    ...
    print(f"4xx 客戶端錯誤: {client_error_count} 次 ({client_error_count/total_requests*100:.2f}%)")
    print(f"5xx 服務器錯誤: {server_error_count} 次 ({server_error_count/total_requests*100:.2f}%)")
    
    # 6.2 錯誤 URL 統計
    print("\n6.2 產生錯誤的 URL:")
    error_urls = parsed_logs.filter(lambda x: x['status_code'] >= 400) \
    ...
    minute_counts = parsed_logs.map(lambda x: (extract_minute(x['timestamp']), 1)) \
                              .reduceByKey(lambda a, b: a + b) \
                              .sortByKey()
    ...
    user_sessions = parsed_logs.map(lambda x: (x['ip'], x)) \
                              .groupByKey() \
    ...
    # 檢測大量 404 錯誤 (可能是掃描攻擊)
    suspicious_404 = parsed_logs.filter(lambda x: x['status_code'] == 404) \
                               .map(lambda x: (x['ip'], 1)) \
                               .reduceByKey(lambda a, b: a + b) \
                               .filter(lambda x: x[1] >= 2)  # 2次以上404錯誤
    ...
    # 找出響應大小超過平均值5倍的請求
    large_responses = parsed_logs.filter(lambda x: x['response_size'] > avg_response_size * 5)
    ...
    # 10.1 成功率統計
    success_requests = parsed_logs.filter(lambda x: 200 <= x['status_code'] < 300).count()
    ...
    api_success_rates = api_logs.map(lambda x: (x['url'], (1 if 200 <= x['status_code'] < 300 else 0, 1))) \
