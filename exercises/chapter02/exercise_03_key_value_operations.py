...
def main():
    ...
    print("各產品總銷售額:")
    for product, total in total_sales_by_product.sortBy(lambda x: x[1], ascending=False).collect():
    ...
    # 按產品分組並計算統計
    product_details = sales_rdd.map(lambda x: (x[0], [x[1]])) \
    ...
    region_counts = region_sales.map(lambda x: (x[0], 1)).reduceByKey(lambda a, b: a + b)
    
    # 合併地區統計
    region_stats = region_totals.join(region_counts) \
                               .mapValues(lambda x: {'total': x[0], 'count': x[1], 'avg': x[0]/x[1]})
    
    print("各地區銷售統計:")
    for region, stats in region_stats.sortBy(lambda x: x[1]['total'], ascending=False).collect():
        print(f"{region}:")
        print(f"  - 總銷售額: ${stats['total']:,.2f}")
        print(f"  - 銷售次數: {stats['count']}")
        print(f"  - 平均銷售額: ${stats['avg']:,.2f}")
    ...
    print("產品-地區銷售額:")
    for (product, region), total in product_region_totals.sortBy(lambda x: x[1], ascending=False).collect():
    ...
    partition_counts = partitioned_sales.mapPartitionsWithIndex(partition_stats).collect()
    print("每個分區的記錄數:")
    ...
    # 使用廣播變量轉換地區名稱
    sales_with_codes = sales_rdd.map(lambda x: (
        x[0], 
    ...
    region_product_sales = sales_rdd.map(lambda x: (x[1][2], (x[0], x[1][0]))) \
    ...
    # 按產品和日期分組
    product_daily_sales = sales_rdd.map(lambda x: ((x[0], x[1][1]), x[1][0])) \
                                  .reduceByKey(lambda a, b: a + b)
    
    # 按產品分組日期銷售
    product_timeline = product_daily_sales.map(lambda x: (x[0][0], (x[0][1], x[1]))) \
                                         .groupByKey() \
    ...
    # 11.2 合併操作減少 shuffle
    combined_stats = sales_rdd.map(lambda x: (x[0], (x[1][0], 1))) \
                             .reduceByKey(lambda a, b: (a[0] + b[0], a[1] + b[1])) \
                             .mapValues(lambda x: {'total': x[0], 'count': x[1], 'avg': x[0]/x[1]})
    
    print("\n優化後的產品統計:")
    for product, stats in combined_stats.sortByKey().collect():
        print(f"{product}: 總額 ${stats['total']:,.2f}, 次數 {stats['count']}, 平均 ${stats['avg']:,.2f}")
