...
from pyspark.sql.functions import avg, col, count, from_json, split, window
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("Streaming基礎練習")
        .master("local[*]")
    ...
    # 添加計算欄位
    transformed_stream = rate_stream.withColumn(
        "doubled_value", col("value") * 2
    ...
    # 模擬 JSON 數據流
    json_data_stream = rate_stream.withColumn(
        "json_data", col("value").cast("string").alias("user_id")
    ).select(
        col("timestamp"),
    ...
    # 按時間窗口聚合
    windowed_counts = json_data_stream.groupBy(
        window(col("timestamp"), "10 seconds", "5 seconds"), col("product_id")
    ).agg(count("*").alias("order_count"), avg("price").alias("avg_price"))
