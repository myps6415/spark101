...
from pyspark.sql.functions import (array_contains, avg, col, collect_list,
                                   concat_ws, count, current_timestamp,
                                   date_format, explode, expr, from_json)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import regexp_extract, split, struct
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("Kafka實時數據管道練習")
        .master("local[*]")
    ...
    kafka_messages = kafka_like_stream.select(
        ...
        when(
            ...
            concat_ws(
                ...
                lit('",'),
                lit(' "session_id": "sess_'),
                (col("value") % 100).cast("string"),
                lit('"}'),
            ),
        )
        .when(
            ...
            concat_ws(
                ...
                lit(","),
                lit(' "amount": '),
                (col("value") % 1000 + 10).cast("string"),
                lit(","),
                lit(' "currency": "USD",'),
                lit(' "status": "'),
                when((col("value") % 10) < 8, "completed").otherwise("failed"),
                lit('",'),
                lit(' "timestamp": "'),
                col("timestamp").cast("string"),
                lit('"}'),
        ...
        .otherwise(
            concat_ws(
                "",
                lit('{"sensor_id": "sensor_'),
                (col("value") % 10).cast("string"),
                lit('",'),
                lit(' "temperature": '),
                (20 + (col("value") % 20)).cast("string"),
                lit(","),
                lit(' "humidity": '),
                (40 + (col("value") % 40)).cast("string"),
                lit(","),
                lit(' "location": "warehouse_'),
                ((col("value") % 5) + 1).cast("string"),
                lit('",'),
                lit(' "timestamp": "'),
                col("timestamp").cast("string"),
                lit('"}'),
    ...
    # 解析用戶事件
    user_events = (
        kafka_messages.filter(col("topic") == "user_events")
        .select(
    ...
    # 用戶事件聚合
    user_event_aggregation = (
        user_events.withWatermark("kafka_timestamp", "2 minutes")
        .groupBy(window(col("kafka_timestamp"), "1 minute"), col("event_type"))
        .agg(
    ...
    # 啟動用戶事件處理
    user_event_query = (
        user_event_aggregation.writeStream.outputMode("update")
        .format("console")
    ...
    # 解析交易事件
    transaction_events = (
        kafka_messages.filter(col("topic") == "transaction_events")
        .select(
            col("timestamp").alias("kafka_timestamp"),
            from_json(col("value"), transaction_schema).alias("data"),
        )
        .select(col("kafka_timestamp"), col("data.*"))
    )

    # 交易實時監控
    transaction_monitoring = (
        transaction_events.withWatermark("kafka_timestamp", "1 minute")
        .groupBy(window(col("kafka_timestamp"), "30 seconds"), col("status"))
        .agg(
        ...
        .withColumn("amount_per_user", col("total_amount") / col("unique_users"))
    )

    # 啟動交易監控
    transaction_query = (
        transaction_monitoring.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
        .trigger(processingTime="5 seconds")
        .start()
    ...
    user_transaction_join = (
        ...
        .join(
            ...
            & (
                ...
                <= col("u.kafka_timestamp") + expr("interval 5 minutes")
            ),
            "inner",
        )
        .select(
    ...
    # 用戶轉化分析
    conversion_analysis = (
        user_transaction_join.withWatermark("user_event_time", "5 minutes")
        .groupBy(window(col("user_event_time"), "2 minutes"), col("event_type"))
        .agg(
        ...
        .withColumn(
            "revenue_per_user", col("revenue_from_event_type") / col("converting_users")
        )
    )

    # 啟動轉化分析
    conversion_query = (
        conversion_analysis.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    system_logs = (
        kafka_messages.filter(col("topic") == "system_logs")
        .select(
            col("timestamp").alias("kafka_timestamp"),
    ...
    # 日誌聚合和告警
    log_aggregation = (
        system_logs.withWatermark("kafka_timestamp", "1 minute")
        .groupBy(
            window(col("kafka_timestamp"), "1 minute"), col("service"), col("level")
        )
        .agg(
    ...
    # 啟動日誌監控
    log_query = (
        log_aggregation.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    # 解析傳感器數據
    sensor_data = (
        kafka_messages.filter(col("topic") == "sensor_data")
        .select(
            col("timestamp").alias("kafka_timestamp"),
    ...
    sensor_anomaly_detection = (
        sensor_data.withWatermark("kafka_timestamp", "2 minutes")
        .groupBy(window(col("kafka_timestamp"), "1 minute"), col("location"))
        .agg(
    ...
    # 啟動傳感器監控
    sensor_query = (
        sensor_anomaly_detection.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    # 創建匯總數據用於輸出
    business_summary = (
        kafka_messages.withWatermark("timestamp", "2 minutes")
        .groupBy(window(col("timestamp"), "1 minute"), col("topic"))
    ...
    # 輸出到控制台
    console_query = (
        business_summary.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    # 配置容錯處理
    fault_tolerant_query = (
        fault_tolerant_processing.writeStream.outputMode("update")
        .format("console")
    ...
    optimized_processing = (
        kafka_messages.coalesce(2)
        .withWatermark("timestamp", "1 minute")
        .groupBy(window(col("timestamp"), "30 seconds"), col("topic"))
        .agg(
    ...
    # 7.2 監控指標收集
    monitoring_query = (
        optimized_processing.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
