...
from pyspark.sql.functions import (array_contains, avg, col, collect_list,
                                   count, current_timestamp, dense_rank,
                                   explode, expr, first, lag, last, lead)
from pyspark.sql.functions import max as spark_max
from pyspark.sql.functions import min as spark_min
from pyspark.sql.functions import row_number, size, split, struct
from pyspark.sql.functions import sum as spark_sum
from pyspark.sql.functions import to_timestamp, unix_timestamp, when, window
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("狀態管理和複雜事件處理練習")
        .master("local[*]")
        .config(
            "spark.sql.streaming.checkpointLocation", "/tmp/spark-stateful-checkpoint"
        )
    ...
    enriched_events = (
        ...
        .withColumn(
            "event_value",
            when(
                col("event_type").isin("add_to_cart", "checkout_complete"),
                col("session_value"),
            ).otherwise(0),
        )
    )
    ...
    # 會話狀態聚合
    session_aggregation = (
        enriched_events.withWatermark("timestamp", "2 minutes")
        .groupBy("user_id")
        .agg(
            ...
            spark_sum(when(col("event_type") == "product_view", 1).otherwise(0)).alias(
                "product_views"
            ),
            spark_sum(when(col("event_type") == "add_to_cart", 1).otherwise(0)).alias(
                "add_to_cart_events"
            ),
            spark_sum(
                when(
                    col("event_type").isin("checkout_complete", "payment_success"), 1
                ).otherwise(0)
            ).alias("conversions"),
        ...
        .withColumn("conversion_rate", col("conversions") / col("total_events") * 100)
        .withColumn(
            "user_segment",
    ...
    # 啟動會話聚合查詢
    session_query = (
        session_aggregation.writeStream.outputMode("update")
        .format("console")
    ...
    # 檢測完整購買路径
    purchase_funnel_analysis = (
        funnel_events.withWatermark("timestamp", "5 minutes")
        .groupBy(window(col("timestamp"), "10 minutes"), col("user_id"))
        .agg(
            collect_list(struct("timestamp", "funnel_step", "event_type")).alias(
                "funnel_events"
            ),
        ...
        .withColumn("is_successful_conversion", col("max_funnel_step") == 4)
    )

    # 啟動漏斗分析查詢
    funnel_query = (
        purchase_funnel_analysis.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    # 檢測異常行為模式
    anomaly_detection = (
        enriched_events.withWatermark("timestamp", "3 minutes")
        .groupBy(window(col("timestamp"), "2 minutes"), col("user_id"))
        .agg(
            ...
            spark_sum(
                when(col("event_type") == "payment_failed", 1).otherwise(0)
        ...
        .withColumn(
            ...
            .when(
                col("cart_removals") > col("cart_additions"), "Negative Cart Behavior"
            )
            .otherwise("Normal"),
        )
    ...
    # 啟動異常檢測查詢
    anomaly_query = (
        anomaly_detection.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    user_interest_analysis = (
        enriched_events.filter(col("event_type").isin("product_view", "add_to_cart"))
    ...
    # 4.2 協同過濾推薦
    collaborative_filtering = user_interest_analysis.select(
        "user_id", "recommendation_category", "user_preference_score"
    ).withColumn(
        "similar_users_products",
        when(
            col("recommendation_category") == "Electronics",
            array(lit(101), lit(102), lit(103)),
        )
        .when(
            col("recommendation_category") == "Fashion",
            array(lit(201), lit(202), lit(203)),
        )
        .when(
            col("recommendation_category") == "Home & Garden",
            array(lit(301), lit(302), lit(303)),
        )
    ...
    recommendation_query = (
        collaborative_filtering.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    # 業務關鍵指標
    business_kpis = (
        enriched_events.withWatermark("timestamp", "1 minute")
        .groupBy(window(col("timestamp"), "1 minute"))
        .agg(
            count("*").alias("total_events"),
            expr("approx_count_distinct(user_id)").alias("active_users"),
            spark_sum(
                when(col("event_type") == "payment_success", 1).otherwise(0)
            ).alias("successful_purchases"),
            spark_sum(
                when(col("event_type") == "payment_failed", 1).otherwise(0)
        ...
        .withColumn(
            ...
            col("successful_purchases")
            / (col("successful_purchases") + col("failed_payments"))
            * 100,
        )
    )
    ...
    # 5.2 產品熱度分析
    product_popularity = (
        enriched_events.filter(
            col("event_type").isin("product_view", "add_to_cart", "payment_success")
        )
        ...
        .withColumn(
            "product_score", col("views") + col("cart_adds") * 2 + col("purchases") * 5
        )
        .withColumn(
            "popularity_rank",
            row_number().over(
                Window.partitionBy("window").orderBy(col("product_score").desc())
            ),
        )
    ...
    # 啟動KPI監控查詢
    kpi_query = (
        business_kpis.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    fraud_detection = (
        enriched_events.withWatermark("timestamp", "5 minutes")
        .groupBy("user_id")
        .agg(
            count("*").alias("total_events"),
            spark_sum(
        ...
        .withColumn(
            "fraud_risk_score",
            when(col("payment_failures") > 3, 50).otherwise(0)
            + when(col("avg_transaction_value") > 500, 30).otherwise(0)
            + when(col("total_events") > 50, 20).otherwise(0),
        )
        .withColumn(
            "fraud_risk_level",
    ...
    # 啟動欺詐檢測查詢
    fraud_query = (
        fraud_detection.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
        .trigger(processingTime="8 seconds")
        .start()
    ...
    customer_lifecycle = (
        enriched_events.withWatermark("timestamp", "10 minutes")
        .groupBy("user_id")
        .agg(
            count("*").alias("engagement_score"),
            spark_sum("event_value").alias("monetary_value"),
            expr("approx_count_distinct(date_format(timestamp, 'yyyy-MM-dd'))").alias(
                "active_days"
            ),
            spark_sum(
                when(col("event_type") == "payment_success", 1).otherwise(0)
        ...
        .withColumn(
            "customer_tier",
            when(
                (col("monetary_value") > 1000) & (col("purchase_frequency") > 2), "VIP"
            )
            .when(
                (col("monetary_value") > 500) & (col("purchase_frequency") > 1),
                "Premium",
            )
    ...
    # 啟動客戶生命週期分析
    lifecycle_query = (
        customer_lifecycle.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
        .trigger(processingTime="10 seconds")
        .start()
    ...
    temporal_analysis = (
        enriched_events.withColumn("hour_of_day", expr("hour(timestamp)"))
        .withColumn(
            "day_period",
        ...
        .groupBy(
            window(col("timestamp"), "10 minutes"), col("day_period"), col("event_type")
        )
        .agg(
    ...
    channel_analysis = (
        ...
        .groupBy(window(col("timestamp"), "5 minutes"), col("channel_type"))
        .agg(
            count("*").alias("events"),
            expr("approx_count_distinct(user_id)").alias("users"),
            spark_sum("event_value").alias("revenue"),
        )
        .withColumn("revenue_per_user", col("revenue") / col("users"))
    )

    # 啟動多維度分析
    temporal_query = (
        temporal_analysis.writeStream.outputMode("update")
        .format("console")
        .option("truncate", False)
    ...
    business_alerts = (
        enriched_events.withWatermark("timestamp", "30 seconds")
        .groupBy(window(col("timestamp"), "1 minute"))
        .agg(
            count("*").alias("total_events"),
            spark_sum(
            ...
            spark_sum(
                when(col("event_type") == "session_timeout", 1).otherwise(0)
            ).alias("session_timeouts"),
            expr("approx_count_distinct(user_id)").alias("active_users"),
        )
        .withColumn(
            "payment_failure_rate", col("payment_failures") / col("total_events") * 100
        )
        .withColumn(
            "alert_type",
            when(col("payment_failure_rate") > 10, "HIGH_PAYMENT_FAILURE_RATE")
            .when(
                col("session_timeouts") > col("active_users") * 0.3,
                "HIGH_SESSION_TIMEOUT_RATE",
            )
    ...
    # 啟動告警系統
    alert_query = (
        business_alerts.writeStream.outputMode("append")
        .format("console")
        .option("truncate", False)
        .trigger(processingTime="5 seconds")
        .start()
