...
from pyspark.sql.functions import col, window, count, avg, sum as spark_sum, max as spark_max, \
    min as spark_min, expr, when, current_timestamp, to_timestamp, \
...
def main():
    ...
    def generate_sensor_data():
        ...
        for i in range(100):
            ...
            for sensor_id in ["sensor_001", "sensor_002", "sensor_003"]:
                location = {"sensor_001": "warehouse_A", "sensor_002": "warehouse_B", "sensor_003": "warehouse_C"}[sensor_id]
    ...
    # 10秒固定窗口
    fixed_window_aggregation = enriched_stream \
        .groupBy(
    ...
    # 啟動固定窗口查詢
    fixed_window_query = fixed_window_aggregation.writeStream \
        .outputMode("update") \
        .format("console") \
    ...
    # 20秒窗口，每5秒滑動
    sliding_window_aggregation = enriched_stream \
        .groupBy(
            ...
            expr("aggregate(temperature_readings, 0.0, (acc, x) -> acc + (x - avg_temperature) * (x - avg_temperature)) / size(temperature_readings)")
        )
    
    # 啟動滑動窗口查詢
    sliding_window_query = sliding_window_aggregation.writeStream \
        .outputMode("update") \
        .format("console") \
        .option("truncate", False) \
    ...
    # 使用watermark處理延遲數據
    session_like_aggregation = enriched_stream \
        ...
        .groupBy(
            ...
            expr("cast((session_end.getTime() - session_start.getTime()) / 1000 as int)")
        )
    ...
    # 根據溫度閾值創建事件窗口
    temperature_events = enriched_stream \
        .withColumn(
            "temperature_alert",
    ...
    # 溫度異常事件聚合
    alert_window_aggregation = temperature_events \
        .groupBy(
            window(col("timestamp"), "1 minute"),
            col("temperature_alert"),
            col("location")
        ).agg(
    ...
    # 啟動溫度告警窗口查詢
    alert_query = alert_window_aggregation.writeStream \
        .outputMode("update") \
        .format("console") \
        .option("truncate", False) \
    ...
    # 模擬延遲數據
    delayed_data_stream = enriched_stream \
        .withColumn(
            "event_timestamp",
            # 模擬一些數據延遲5-15秒到達
            when(col("sequence_number") % 10 == 0, 
                 expr("timestamp - interval 10 seconds"))
            .otherwise(col("timestamp"))
        )
    
    # 使用watermark處理延遲數據
    late_data_aggregation = delayed_data_stream \
        .withWatermark("event_timestamp", "20 seconds") \
        .groupBy(
            window(col("event_timestamp"), "15 seconds"),
            col("location")
        ).agg(
            count("*").alias("event_count"),
            avg("temperature").alias("avg_temperature"),
            expr("count(case when timestamp != event_timestamp then 1 end)").alias("late_arrivals")
        )
    
    # 啟動延遲數據處理查詢
    late_data_query = late_data_aggregation.writeStream \
        .outputMode("update") \
        .format("console") \
        .option("truncate", False) \
        .trigger(processingTime='5 seconds') \
    ...
    # 第一級：秒級聚合
    second_level_agg = enriched_stream \
        .groupBy(
            window(col("timestamp"), "10 seconds"),
            col("sensor_name"),
            col("location")
    ...
    minute_level_agg = enriched_stream \
        .groupBy(
            window(col("timestamp"), "1 minute"),
            col("location")
        ).agg(
    ...
    # 啟動分層聚合查詢
    minute_agg_query = minute_level_agg.writeStream \
        .outputMode("update") \
        .format("console") \
        .option("truncate", False) \
        .trigger(processingTime='10 seconds') \
    ...
    # 檢測連續上升的溫度模式
    temperature_trends = enriched_stream \
        .select(
            col("timestamp"),
            col("sensor_name"),
            col("location"),
            col("temperature"),
            expr("lag(temperature) over (partition by sensor_name order by timestamp)").alias("prev_temperature"),
            expr("lag(temperature, 2) over (partition by sensor_name order by timestamp)").alias("prev2_temperature")
        ).filter(
    ...
    # 6.2 異常模式聚合
    pattern_aggregation = temperature_trends \
        .filter(col("trend_pattern") != "STABLE") \
        .groupBy(
    ...
    # 啟動模式檢測查詢
    pattern_query = pattern_aggregation.writeStream \
        .outputMode("update") \
        .format("console") \
        .option("truncate", False) \
    ...
    cumulative_stats = enriched_stream \
        .groupBy("sensor_name", "location") \
        .agg(
    ...
    # 7.2 運行平均計算
    running_averages = enriched_stream \
        .groupBy(
            window(col("timestamp"), "20 seconds", "5 seconds"),
            col("sensor_name")
        ).agg(
    ...
    # 啟動運行平均查詢
    running_avg_query = running_averages.writeStream \
        .outputMode("update") \
        .format("console") \
        .option("truncate", False) \
    ...
    sensor_config_df = spark.createDataFrame(sensor_config_data, config_schema) \
    ...
    enriched_with_config = enriched_stream.join(
        ...
        "temperature_status",
        when(col("temperature") > col("threshold_temp"), "ABOVE_THRESHOLD")
        .otherwise("NORMAL")
    )
    
    # 基於配置的告警聚合
    config_based_alerts = enriched_with_config \
        .filter(col("temperature_status") == "ABOVE_THRESHOLD") \
        .groupBy(
            window(col("timestamp"), "45 seconds"),
            col("location"),
            col("priority")
        ).agg(
            count("*").alias("alert_count"),
    ...
    # 啟動配置基礎告警查詢
    config_alert_query = config_based_alerts.writeStream \
        .outputMode("update") \
        .format("console") \
        .option("truncate", False) \
        .trigger(processingTime='10 seconds') \
    ...
    # 根據數據頻率調整窗口大小
    adaptive_windows = enriched_stream \
        .groupBy(
    ...
    # 9.2 多時間尺度分析
    multi_scale_analysis = enriched_stream \
        .select(
            col("timestamp"),
            col("sensor_name"),
            col("location"),
            ...
            "short_term_anomaly",
            expr("abs(temperature - temp_10sec_avg) > 2")
        ).withColumn(
            "long_term_trend",
    ...
    # 啟動多尺度分析查詢
    multi_scale_query = multi_scale_analysis.writeStream \
        .outputMode("append") \
        .format("console") \
        .option("truncate", False) \
        .trigger(processingTime='8 seconds') \
    ...
    # 10.1 批處理間隔優化
    optimized_aggregation = enriched_stream \
        .coalesce(2) \
        .groupBy(
            window(col("timestamp"), "30 seconds"),
            col("location")
    ...
    # 10.2 檢查點和容錯
    checkpoint_query = optimized_aggregation.writeStream \
        .outputMode("update") \
        .format("console") \
