...
def main():
    ...
    user_behavior_schema = StructType([
        ...
        StructField("event_type", StringType(), True),  # view, click, cart, purchase, search
    ...
    transaction_schema = StructType([
        ...
        StructField("status", StringType(), True),  # pending, paid, shipped, delivered, cancelled
        StructField("timestamp", TimestampType(), True),
    ...
    class EcommerceDataGenerator:
        ...
        def generate_user_behavior_stream(self):
            ...
            # 生成用戶行為事件
            behavior_stream = base_stream.select(
                col("timestamp"),
                ...
                "user_id", concat_ws("", lit("user_"), col("user_idx") + 1)
            ).withColumn(
                "product_id", concat_ws("", lit("product_"), col("product_idx") + 1)
            ).withColumn(
                "category_id", concat_ws("", lit("category_"), col("category_idx") + 1)
            ).withColumn(
                "brand_id", concat_ws("", lit("brand_"), col("brand_idx") + 1)
            ).withColumn(
                "session_id", concat_ws("", col("user_id"), lit("_session_"), col("session_idx"))
            ).withColumn(
                "event_type",
                ...
                "price", 
                (col("sequence") % 1000 + 10).cast("double")
            ).withColumn(
                "quantity",
                ...
                "page_url", 
                concat_ws("", lit("/product/"), col("product_id"))
            ).withColumn(
                "referrer",
        ...
        def generate_transaction_stream(self):
            ...
            transaction_stream = base_stream.select(
                ...
                "order_id", concat_ws("", lit("order_"), col("value") // 3)
            ).withColumn(
                "product_id", concat_ws("", lit("product_"), (col("value") % 5000) + 1)
            ).withColumn(
                "quantity", (col("value") % 5) + 1
            ).withColumn(
                "unit_price", (col("value") % 1000 + 10).cast("double")
            ).withColumn(
                ...
                "promotion_code",
                when(col("value") % 50 == 0, concat_ws("", lit("PROMO"), (col("value") % 100)))
                .otherwise(None)
    ...
    class UserBehaviorAnalyzer:
            ...
            metrics = behavior_stream \
                ...
                .groupBy(
                    ...
                    when(col("event_type") == "purchase", 
                         col("event_count") / lag("event_count").over(
                             Window.partitionBy("window", "device_type", "location")
                                   .orderBy("event_type")))
                    .otherwise(0.0)
                )
        ...
        def user_session_analysis(self, behavior_stream):
            ...
            session_analysis = behavior_stream \
                ...
                .groupBy(
                    ...
                    expr("approx_count_distinct(category_id)").alias("categories_viewed"),
                    collect_list("event_type").alias("event_sequence"),
                    spark_sum(when(col("event_type") == "cart", col("quantity")).otherwise(0)).alias("cart_items"),
                    spark_sum(when(col("event_type") == "purchase", col("price") * col("quantity")).otherwise(0)).alias("purchase_value"),
        ...
        def product_performance_analysis(self, behavior_stream):
            ...
            product_metrics = behavior_stream \
                ...
                .groupBy(
                    ...
                    spark_sum(when(col("event_type") == "cart", 1).otherwise(0)).alias("cart_adds"),
                    spark_sum(when(col("event_type") == "purchase", 1).otherwise(0)).alias("purchases"),
                    spark_sum(when(col("event_type") == "purchase", col("quantity")).otherwise(0)).alias("units_sold"),
                    spark_sum(when(col("event_type") == "purchase", col("price") * col("quantity")).otherwise(0)).alias("revenue"),
                    expr("approx_count_distinct(user_id)").alias("unique_viewers")
                ).withColumn(
                    "click_through_rate",
                    when(col("views") > 0, col("clicks") / col("views") * 100).otherwise(0)
                ).withColumn(
                    "cart_conversion_rate", 
                    when(col("clicks") > 0, col("cart_adds") / col("clicks") * 100).otherwise(0)
                ).withColumn(
                    "purchase_conversion_rate",
                    when(col("cart_adds") > 0, col("purchases") / col("cart_adds") * 100).otherwise(0)
                ).withColumn(
                    "revenue_per_view",
    ...
    class SalesAnalyzer:
        ...
        def real_time_sales_metrics(self, transaction_stream):
            ...
            sales_metrics = transaction_stream \
                .filter(col("status").isin("paid", "shipped", "delivered")) \
        ...
        def fraud_detection(self, transaction_stream):
            ...
            user_stats = transaction_stream \
                ...
                .groupBy(
                    ...
                    expr("approx_count_distinct(payment_method)").alias("unique_payment_methods"),
                    expr("approx_count_distinct(shipping_address)").alias("unique_addresses")
                )
            
            # 欺詐風險評分
            fraud_scores = user_stats.withColumn(
                "velocity_risk",
        ...
        def revenue_forecasting(self, sales_metrics):
            ...
            forecast_analysis = sales_metrics \
                .withColumn("revenue_trend", 
                           lag("total_revenue", 1).over(window_spec)) \
                .withColumn("revenue_growth_rate",
                           when(col("revenue_trend") > 0,
                                (col("total_revenue") - col("revenue_trend")) / col("revenue_trend") * 100)
                           .otherwise(0)) \
                .withColumn("revenue_ma_5",
                           avg("total_revenue").over(window_spec.rowsBetween(-4, 0))) \
                .withColumn("predicted_next_period",
    ...
    class RecommendationEngine:
        ...
        def prepare_training_data(self, behavior_stream):
            ...
            # 從用戶行為中提取隱式評分
            implicit_ratings = behavior_stream \
                .filter(col("event_type").isin("view", "click", "cart", "purchase")) \
            ...
            indexed_ratings = user_indexer.fit(implicit_ratings).transform(implicit_ratings)
        ...
        def content_based_recommendations(self, behavior_stream):
            ...
            # 分析用戶偏好
            user_preferences = behavior_stream \
                .filter(col("event_type").isin("view", "purchase", "cart")) \
        ...
        def real_time_recommendations(self, behavior_stream, user_preferences):
            ...
            # 基於最近行為的推薦
            recent_activity = behavior_stream \
                .withWatermark("timestamp", "30 minutes") \
            ...
            # 結合用戶偏好生成推薦
            recommendations = recent_activity.join(user_preferences, "user_id", "left") \
                .withColumn("recommended_categories",
                           when(col("preferred_categories").isNotNull(),
                                col("preferred_categories"))
                           .otherwise(col("recent_categories"))) \
                .withColumn("recommendation_type",
                           when(col("preferred_categories").isNotNull(), "personalized")
                           .otherwise("trending"))
    ...
    class InventoryManager:
        ...
        def inventory_monitoring(self, transaction_stream):
            ...
            # 計算產品需求
            demand_analysis = transaction_stream \
                .filter(col("status").isin("paid", "shipped")) \
                .withWatermark("timestamp", "30 minutes") \
                .groupBy(
                    window(col("timestamp"), "15 minutes", "5 minutes"),
                    col("product_id")
                    ...
                    "demand_trend",
                    lag("demand_quantity").over(Window.partitionBy("product_id").orderBy("window"))
                ).withColumn(
                    "demand_growth",
                    when(col("demand_trend") > 0,
                         (col("demand_quantity") - col("demand_trend")) / col("demand_trend") * 100)
                    .otherwise(0)
                )
        ...
        def reorder_alerts(self, demand_analysis):
            ...
            # 模擬當前庫存狀態
            current_inventory = demand_analysis \
                .withColumn("estimated_stock", 
                           lit(1000) - col("demand_quantity") * 2) \
                .withColumn("reorder_point", lit(100)) \
                .withColumn("max_stock", lit(1000)) \
                .withColumn("lead_time_days", lit(7))
            
            # 補貨建議
            reorder_recommendations = current_inventory \
                .withColumn("days_of_supply",
                           when(col("demand_quantity") > 0,
                                col("estimated_stock") / col("demand_quantity") * 0.25)  # 15分鐘窗口轉天數
                           .otherwise(999)) \
                .withColumn("reorder_quantity",
                           when(col("estimated_stock") <= col("reorder_point"),
    ...
    class AnomalyDetectionSystem:
        ...
        def system_anomaly_detection(self, behavior_stream, transaction_stream):
            ...
            # 系統指標異常檢測
            system_metrics = behavior_stream \
            ...
            anomaly_scores = system_metrics \
                .withColumn("events_ma", 
                           avg("total_events").over(window_spec.rowsBetween(-4, 0))) \
                .withColumn("events_std",
                           stddev("total_events").over(window_spec.rowsBetween(-4, 0))) \
                .withColumn("events_zscore",
                           when(col("events_std") > 0,
                                spark_abs(col("total_events") - col("events_ma")) / col("events_std"))
                           .otherwise(0)) \
                .withColumn("users_ma",
                           avg("active_users").over(window_spec.rowsBetween(-4, 0))) \
                .withColumn("users_std", 
                           stddev("active_users").over(window_spec.rowsBetween(-4, 0))) \
                .withColumn("users_zscore",
                           when(col("users_std") > 0,
                                spark_abs(col("active_users") - col("users_ma")) / col("users_std"))
                           .otherwise(0)) \
                .withColumn("anomaly_score",
                           col("events_zscore") + col("users_zscore")) \
        ...
        def business_anomaly_detection(self, transaction_stream):
            ...
            # 業務指標異常
            business_metrics = transaction_stream \
                .withWatermark("timestamp", "10 minutes") \
                ...
                .agg(
                    ...
                    spark_sum(when(col("status") == "pending", 1).otherwise(0)).alias("pending_orders")
                ).withColumn(
                    "cancellation_rate",
            ...
            # 檢測業務異常
            business_anomalies = business_metrics \
                .withColumn("revenue_anomaly",
                           when(col("total_revenue") < lag("total_revenue").over(
                               Window.orderBy("window")) * 0.5, "LOW_REVENUE")
                           .when(col("total_revenue") > lag("total_revenue").over(
                               Window.orderBy("window")) * 2, "HIGH_REVENUE")
                           .otherwise("NORMAL")) \
                .withColumn("cancellation_anomaly",
                           when(col("cancellation_rate") > 20, "HIGH_CANCELLATION")
                           .otherwise("NORMAL")) \
                .withColumn("alert_level",
                           when((col("revenue_anomaly") != "NORMAL") | 
                                (col("cancellation_anomaly") != "NORMAL"), "ALERT")
                           .otherwise("OK")) \
    ...
    class EcommercePipelineOrchestrator:
        ...
        def start_pipeline(self):
            ...
            session_analysis = self.behavior_analyzer.user_session_analysis(behavior_stream)
            product_performance = self.behavior_analyzer.product_performance_analysis(behavior_stream)
            
            # 銷售分析
            sales_metrics = self.sales_analyzer.real_time_sales_metrics(transaction_stream)
            ...
            # 推薦系統
            user_preferences = self.recommendation_engine.content_based_recommendations(behavior_stream)
            recommendations = self.recommendation_engine.real_time_recommendations(behavior_stream, user_preferences)
            
            # 庫存管理
            demand_analysis = self.inventory_manager.inventory_monitoring(transaction_stream)
            reorder_alerts = self.inventory_manager.reorder_alerts(demand_analysis)
            
            # 異常檢測
            system_anomalies = self.anomaly_detector.system_anomaly_detection(behavior_stream, transaction_stream)
            business_anomalies = self.anomaly_detector.business_anomaly_detection(transaction_stream)
            
            # 啟動所有查詢
            queries = []
            
            # 用戶行為監控
            behavior_query = behavior_metrics.writeStream \
                .outputMode("append") \
                .format("console") \
            ...
            # 會話分析
            session_query = session_analysis.writeStream \
                .outputMode("append") \
                .format("console") \
                .option("truncate", False) \
                .option("numRows", 5) \
            ...
            # 銷售監控
            sales_query = sales_metrics.writeStream \
                .outputMode("append") \
                .format("console") \
                .option("truncate", False) \
                .option("numRows", 10) \
            ...
            # 庫存告警
            inventory_query = reorder_alerts.writeStream \
                .outputMode("append") \
                .format("console") \
                .option("truncate", False) \
            ...
            # 異常檢測告警
            anomaly_query = system_anomalies.writeStream \
                .outputMode("append") \
                .format("console") \
                .option("truncate", False) \
                .trigger(processingTime="60 seconds") \
            ...
            for query in queries:
                try:
                    ...
                    if progress:
                        ...
                        print(f"  處理時間: {progress.get('durationMs', {}).get('totalMs', 'N/A')} ms")
                        print(f"  輸入速率: {progress.get('inputRowsPerSecond', 'N/A')} rows/sec")
                        print(f"  處理速率: {progress.get('processedRowsPerSecond', 'N/A')} rows/sec")
