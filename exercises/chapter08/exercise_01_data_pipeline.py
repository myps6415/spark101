...
from pyspark.sql.functions import (avg, col, count, current_timestamp,
                                   date_format, from_unixtime, regexp_extract,
                                   split)
...
def main():
    # 創建 SparkSession
    spark = (
        SparkSession.builder.appName("數據管道練習")
        .master("local[*]")
    ...
    # 員工數據轉換
    print("\n2.2 員工數據轉換:")
    employees_transformed = employees_df.withColumn(
        "salary_grade",
    ...
    print("轉換後員工數據:")
    employees_transformed.select(
        "name", "salary", "salary_grade", "years_experience", "experience_level"
    ).show(5)
    ...
    logs_parsed = (
        logs_df.withColumn(
            "timestamp",
            regexp_extract(col("value"), r"^(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})", 1),
        )
        .withColumn(
            "level", regexp_extract(col("value"), r"\s+(INFO|WARN|ERROR|DEBUG)\s+", 1)
        )
        .withColumn(
            "message",
            regexp_extract(col("value"), r"(INFO|WARN|ERROR|DEBUG)\s+(.+)$", 2),
        )
        .filter(col("level") != "")
    )
    ...
    print("\n3.1 銷售分析:")
    sales_analysis = (
        sales_cleaned.groupBy("order_month", "category")
        .agg(
    ...
    print("\n3.2 員工薪資分析:")
    salary_analysis = (
        employees_transformed.groupBy("department", "salary_grade")
        .agg(
    ...
    sales_quality = sales_cleaned.agg(
        ...
        spark_sum(when(col("total_amount") > 100000, 1).otherwise(0)).alias(
            "high_value_orders"
        ),
    )
    sales_quality.show()
    ...
    employee_quality = employees_transformed.agg(
        ...
        spark_sum(when(col("performance_rating") < 1, 1).otherwise(0)).alias(
            "invalid_rating"
        ),
    )
    ...
    # 創建產品維度表
    product_lookup = sales_cleaned.select(
        "product_id", "product_name", "category"
    ).distinct()

    # 豐富化銷售數據
    enriched_sales = sales_cleaned.join(
        product_lookup, ["product_id", "product_name", "category"]
    ).withColumn(
        "revenue_tier",
    ...
    enriched_sales.select(
        "order_id", "customer_name", "total_amount", "revenue_tier"
    ).show(10)
    ...
    # 保存銷售分析結果
    sales_analysis.coalesce(1).write.mode("overwrite").option("header", "true").csv(
        f"{output_base}/sales_analysis"
    )

    # 保存員工分析結果
    salary_analysis.coalesce(1).write.mode("overwrite").option("header", "true").csv(
        f"{output_base}/salary_analysis"
    )

    # 保存豐富化數據為 Parquet
    enriched_sales.write.mode("overwrite").partitionBy("order_month").parquet(
        f"{output_base}/enriched_sales"
    )
