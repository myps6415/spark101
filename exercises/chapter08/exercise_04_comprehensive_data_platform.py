...
from pyspark.sql.functions import (
    ...
    window, current_timestamp, to_timestamp, date_format, regexp_extract,
    explode, split, size, array_contains, struct, collect_list,
    lag, lead, row_number, rank, dense_rank, ntile,
    udf, broadcast, coalesce, isnan, isnull
)
...
class ComprehensiveDataPlatform:
    ...
    def create_sample_datasets(self) -> Dict[str, Any]:
        ...
        users_data = [
            ...
            (3, "Charlie", 35, "Premium", "charlie@example.com", "2023-01-20", "Canada"),
        ...
        users_df = self.spark.createDataFrame(users_data, [
            "user_id", "name", "age", "subscription_tier", "email", "registration_date", "country"
        ])
        ...
        products_df = self.spark.createDataFrame(products_data, [
            "product_id", "product_name", "category", "price", "launch_date", "is_active"
        ])
        ...
        for i in range(1, 1001):  # 1000筆交易
            ...
            base_price = [1299.99, 29.99, 49.99, 15.99, 149.99, 79.99, 199.99, 12.99][product_id - 101]
            amount = base_price * quantity
            
            # 創建時間序列
            days_ago = i % 30
            transaction_time = datetime.now() - timedelta(days=days_ago, hours=i%24, minutes=i%60)
            
            transactions_data.append((
                i, user_id, product_id, quantity, amount, 
                transaction_time.strftime("%Y-%m-%d %H:%M:%S"),
                "completed" if i % 10 != 0 else "failed"
            ))
        
        transactions_df = self.spark.createDataFrame(transactions_data, [
            "transaction_id", "user_id", "product_id", "quantity", "amount", 
        ...
        for i in range(1, 5001):  # 5000條行為記錄
            ...
            behaviors_data.append((
                i, user_id, session_id, action, page_url, 
                timestamp.strftime("%Y-%m-%d %H:%M:%S"),
                f"device_{(i % 3) + 1}"  # device_1, device_2, device_3
            ))
        
        behaviors_df = self.spark.createDataFrame(behaviors_data, [
            "event_id", "user_id", "session_id", "action", "page_url", 
    ...
    def batch_processing_pipeline(self, datasets: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 清理行為數據
        clean_behaviors = behaviors_df.filter(
        ...
        # 產品銷售分析
        product_sales_summary = clean_transactions.join(products_df, "product_id").groupBy(
        ...
        user_behavior_summary = clean_behaviors.groupBy("user_id").agg(
            ...
            spark_sum(when(col("action") == "purchase", 1).otherwise(0)).alias("purchase_events")
        )
        ...
        # 客戶生命週期價值分析
        clv_analysis = user_transaction_summary.join(users_df, "user_id").withColumn(
            "clv_segment",
            ...
            "engagement_score",
            (col("total_transactions") * 0.3 + col("unique_products_purchased") * 0.7)
        )
        
        # 產品推薦矩陣
        product_recommendation_matrix = clean_transactions.join(
            clean_transactions.alias("t2"), "user_id"
        ).filter(
            col("product_id") != col("t2.product_id")
        ).groupBy("product_id", "t2.product_id").agg(
    ...
    def machine_learning_pipeline(self, processed_data: Dict[str, Any]) -> Dict[str, Any]:
        """機器學習管道"""
        print("🤖 執行機器學習管道...")
        ...
        ml_features = clv_data.select(
            ...
            .otherwise(0).alias("tier_score"),
            when(col("clv_segment") == "High Value", 1).otherwise(0).alias("is_high_value")
        ).na.drop()
        
        # 特徵向量化
        feature_cols = ["age", "total_transactions", "total_spent", 
                       "avg_transaction_amount", "unique_products_purchased", 
                       "engagement_score", "tier_score"]
    ...
    def streaming_pipeline(self, datasets: Dict[str, Any]) -> None:
        ...
        # 按時間窗口聚合
        windowed_behaviors = behaviors_df.withColumn(
            "hour", 
            date_format(col("timestamp"), "yyyy-MM-dd HH")
        ).groupBy("hour", "action").agg(
        ...
        anomalies = windowed_behaviors.filter(
            col("action_count") > anomaly_threshold
        )
        ...
        popular_pages = recent_behaviors.groupBy("page_url").agg(
            count("*").alias("visit_count")
        ).orderBy(desc("visit_count")).limit(5)
    ...
    def monitoring_and_alerting(self, processed_data: Dict[str, Any], ml_results: Dict[str, Any]) -> Dict[str, Any]:
        """監控和告警系統"""
        print("📊 監控和告警系統...")
        ...
        quality_metrics = {
            ...
            "users_with_transactions": transactions_summary.filter(col("total_transactions") > 0).count(),
            "high_value_users": transactions_summary.filter(col("total_spent") > 1000).count(),
            "avg_transaction_value": transactions_summary.agg(avg("avg_transaction_amount")).collect()[0][0]
        }
        ...
        model_metrics = {
            ...
            "high_value_predictions": ml_results["classification_predictions"].filter(col("prediction") == 1).count()
        }
        
        # 3. 業務指標監控
        print("  💼 業務指標監控...")
        
        business_metrics = processed_data["product_sales_summary"].agg(
        ...
        # 數據質量告警
        if quality_metrics["users_with_transactions"] / quality_metrics["total_users"] < 0.8:
    ...
    def generate_comprehensive_report(self, datasets: Dict[str, Any], 
        ...
        # 熱門產品
        top_products = processed_data["product_sales_summary"].orderBy(desc("total_revenue")).limit(3)
        print("  🔥 熱門產品TOP3：")
        ...
        for row in cluster_summary.collect():
            print(f"    - 群組 {row.prediction}: {row.cluster_size} 人, 平均消費 ${row.avg_spent:,.2f}")
        ...
        # 5. 監控告警
        print("\n📊 監控狀態：")
        print(f"  • 數據質量得分: {monitoring_report['quality_metrics']['users_with_transactions'] / monitoring_report['quality_metrics']['total_users'] * 100:.1f}%")
        print(f"  • 模型性能: AUC {monitoring_report['model_metrics']['classification_auc']:.3f}")
        print(f"  • 平台總收入: ${monitoring_report['business_metrics']['total_platform_revenue']:,.2f}")
